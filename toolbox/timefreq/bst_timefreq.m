...
for iData = 1:length(Data)
        ...
        switch (DataType)
            case 'data'
                ...
                if isstruct(sMat.F)
                    ...
                    [F, sMat.Time] = in_fread(sFile, ChannelMat, 1, SamplesBounds, [], ImportOptions);
            ...
            case 'results'
                ...
                ResultsMat = in_bst_results(InitFile, isLoadFull, 'ImageGridAmp', 'ImagingKernel', 'GoodChannel', 'nComponents', 'DataFile', 'nAvg', 'Time', 'Atlas', 'SurfaceFile', 'GridLoc', 'GridAtlas', 'Atlas');
    ...
    switch (OPTIONS.Method)
        ...
        case 'psd'
            % Calculate PSD/FFT
            [TF, OPTIONS.Freqs, Nwin, Messages] = bst_psd(F, sfreq, OPTIONS.WinLength, OPTIONS.WinOverlap, BadSegments);
            if isempty(TF)
                return;
            end
            % Keep only first and last time instants
        ...
        case 'hilbert'
            ...
            for iBand = 1:size(BandBounds,1)
                ...
                Fband = process_bandpass('Compute', F, sfreq, BandBounds(iBand,1), BandBounds(iBand,2), [], OPTIONS.isMirror);
    ...
    % Save all the time-frequency maps
        ...
        % Save file
        SaveFile(iTargetStudy, InitFile, DataType, RowNames, TF, OPTIONS, FreqBands, SurfaceFile, GridLoc, GridAtlas, nAvg, Atlas);
    end
    bst_progress('inc', 1);
end
...
if isAverage
    ...
    % Save file
    SaveFile(iTargetStudy, InitFile, DataType, RowNames, TF_avg, OPTIONS, FreqBands, SurfaceFile, GridLoc, GridAtlas, nAvgTotal, Atlas);
end


%% ===== SAVE FILE =====
    function SaveFile(iTargetStudy, DataFile, DataType, RowNames, TF, OPTIONS, FreqBands, SurfaceFile, GridLoc, GridAtlas, nAvgFile, Atlas)
