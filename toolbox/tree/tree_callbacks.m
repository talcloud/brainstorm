...
%% ===== POPUP: STUDIESDB =====
            case {'studydbsubj', 'studydbcond'}
                if ~bst_get('ReadOnly')
                    ...
                    gui_component('MenuItem', jPopup, [], 'Edit protocol', IconLoader.ICON_EDIT,        [], @(h,ev)bst_call(@gui_edit_protocol, 'edit', iProtocol), []);
                    gui_component('MenuItem', jPopup, [], 'New subject',   IconLoader.ICON_SUBJECT_NEW, [], @(h,ev)bst_call(@db_edit_subject), []);
                    gui_component('MenuItem', jPopup, [], 'New folder', IconLoader.ICON_FOLDER_NEW,  [], @(h,ev)bst_call(@db_add_condition, '*'), []);
                    AddSeparator(jPopup);
            ...
            case 'subject'
                ...
                if ~bst_get('ReadOnly') && ((iSubject == 0) || ~sSubject.UseDefaultAnat)
                    ...
                    if ~isempty(sTemplates)
                        ...
                        AddSeparator(jMenuDefaults);
                        gui_component('MenuItem', jMenuDefaults, [], 'Online help', IconLoader.ICON_EXPLORER, [], @(h,ev)bst_call(@web, 'http://neuroimage.usc.edu/brainstorm/Tutorials/DefaultAnatomy', '-browser'), []);
                    end
...
%% ===== POPUP: STUDYSUBJECT =====
            case 'studysubject'
                ...
                if ~bst_get('ReadOnly') && isDirNode
                    gui_component('MenuItem', jPopup, [], 'New folder', IconLoader.ICON_FOLDER_NEW, [], @(h,ev)bst_call(@db_add_condition, sSubject.Name), []);
                end
                % === IMPORT DATA ===
                if ~bst_get('ReadOnly')
                    AddSeparator(jPopup);
            ...
            case {'condition', 'rawcondition'}
                if ~bst_get('ReadOnly')
                    ...
                    if ~isRaw && (length(bstNodes) >= 2)
                        ...
                        gui_component('MenuItem', jPopup, [], 'Group folders', IconLoader.ICON_FUSION, [], @(h,ev)bst_call(@db_group_conditions, ConditionsPaths), []);
                    end
                    % === EXPORT RAW FILE ===
                    if isRaw
                        ...
                        jMenuExport = gui_component('MenuItem', [], [], 'Export to file', IconLoader.ICON_SAVE, [], @(h,ev)bst_call(@export_data, RawFiles), []);
                    end
                end
...
%% ===== POPUP: CHANNEL =====
            case 'channel'
                ...
                % If only one modality
                if (length(DisplayMod) == 1) && ((length(bstNodes) ~= 1) || isempty(Device)) && ~ismember(Device, {'Vectorview306', 'CTF', '4D', 'KIT', 'KRISS', 'BabyMEG'}) && ~ismember(DisplayMod, {'EEG','ECOG','SEEG','NIRS'})
                    gui_component('MenuItem', jPopup, [], 'Display sensors', IconLoader.ICON_CHANNEL, [], @(h,ev)bst_call(@DisplayChannels, bstNodes, DisplayMod{1}, 'scalp'), []);
                ...
                elseif (length(DisplayMod) >= 1)
                    ...
                    if (length(bstNodes) == 1) && ismember(Device, {'Vectorview306', 'CTF', '4D', 'KIT', 'KRISS', 'BabyMEG'})
                        gui_component('MenuItem', jMenuDisplay, [], [Device ' coils'], IconLoader.ICON_CHANNEL, [], @(h,ev)bst_call(@DisplayChannels, bstNodes, Device, 'scalp'), []);
                    ...
                    elseif (length(bstNodes) == 1) && ismember('NIRS', DisplayMod)
                        gui_component('MenuItem', jMenuDisplay, [], 'NIRS cap', IconLoader.ICON_CHANNEL, [], @(h,ev)bst_call(@DisplayChannels, bstNodes, Device, 'scalp'), []);
                        AddSeparator(jMenuDisplay);
                    end
                    % === ITEM: MODALITIES ===
                    ...
                    for iType = 1:length(DisplayMod)
                        ...
                        if ismember(DisplayMod{iType}, {'EEG','ECOG','SEEG'}) && (length(bstNodes) == 1)
                            if ~isempty(sSubject.iScalp)
                                %gui_component('MenuItem', jMenuDisplay, [], [channelTypeDisplay '   (Head)'],          IconLoader.ICON_SURFACE_SCALP,  [], @(h,ev)bst_call(@DisplayChannels, bstNodes, DisplayMod{iType}, 'scalp'), []);
                                gui_component('MenuItem', jMenuDisplay, [], [channelTypeDisplay '   (Head)'],     IconLoader.ICON_SURFACE_SCALP,  [], @(h,ev)bst_call(@DisplayChannels, bstNodes, DisplayMod{iType}, 'scalp', 1), []);
                            end
                            if ~isempty(sSubject.iInnerSkull)
                                %gui_component('MenuItem', jMenuDisplay, [], [channelTypeDisplay '   (Inner skull)'],        IconLoader.ICON_SURFACE_CORTEX, [], @(h,ev)bst_call(@DisplayChannels, bstNodes, DisplayMod{iType}, 'cortex'), []);
                                gui_component('MenuItem', jMenuDisplay, [], [channelTypeDisplay '   (Inner skull)'],   IconLoader.ICON_SURFACE_INNERSKULL, [], @(h,ev)bst_call(@DisplayChannels, bstNodes, DisplayMod{iType}, 'innerskull', 1), []);
                            end
                            if ~isempty(sSubject.iCortex)
                                %gui_component('MenuItem', jMenuDisplay, [], [channelTypeDisplay '   (Cortex)'],        IconLoader.ICON_SURFACE_INNERSKULL, [], @(h,ev)bst_call(@DisplayChannels, bstNodes, DisplayMod{iType}, 'cortex'), []);
                                gui_component('MenuItem', jMenuDisplay, [], [channelTypeDisplay '   (Cortex)'],   IconLoader.ICON_SURFACE_CORTEX, [], @(h,ev)bst_call(@DisplayChannels, bstNodes, DisplayMod{iType}, 'cortex', 1), []);
                            end
                            if ~isempty(sSubject.iAnatomy)
                                %gui_component('MenuItem', jMenuDisplay, [], [channelTypeDisplay '   (MRI 3D)'],        IconLoader.ICON_ANATOMY,        [], @(h,ev)bst_call(@DisplayChannels, bstNodes, DisplayMod{iType}, 'anatomy'), []);
                                gui_component('MenuItem', jMenuDisplay, [], [channelTypeDisplay '   (MRI 3D)'],     IconLoader.ICON_ANATOMY, [], @(h,ev)bst_call(@DisplayChannels, bstNodes, DisplayMod{iType}, 'anatomy', 1), []);
                                gui_component('MenuItem', jMenuDisplay, [], [channelTypeDisplay '   (MRI Viewer)'], IconLoader.ICON_ANATOMY, [], @(h,ev)bst_call(@DisplayChannelsMri, filenameRelative, DisplayMod{iType}), []);
                            end
                        else
                            gui_component('MenuItem', jMenuDisplay, [], channelTypeDisplay, IconLoader.ICON_CHANNEL, [], @(h,ev)bst_call(@DisplayChannels, bstNodes, DisplayMod{iType}, 'scalp'), []);
                        end
                    end
                end
                % === EDIT CHANNEL FILE ===
                ...
                if ismember('EEG', AllMod)
                    jMenuDefaults = gui_component('Menu', jPopup, [], 'Add EEG positions', IconLoader.ICON_CHANNEL, [], [], []);
                    % Get all studies
                    iAllStudies = tree_channel_studies( bstNodes );
                    % Import from file
                    gui_component('MenuItem', jMenuDefaults, [], 'Import from file', IconLoader.ICON_CHANNEL, [], @(h,ev)bst_call(@channel_add_loc, iAllStudies, [], 1), []);
                    AddSeparator(jMenuDefaults);
                    ...
                    for iDir = 1:length(bstDefaults)
                        jMenuDir = gui_component('Menu', jMenuDefaults, [], ['Default: ' bstDefaults(iDir).name], IconLoader.ICON_FOLDER_CLOSE, [], [], []);
                        % Add an item per Template available
                        fList = bstDefaults(iDir).contents;
                        for iFile = 1:length(fList)
                            gui_component('MenuItem', jMenuDir, [], fList(iFile).name, IconLoader.ICON_CHANNEL, [], @(h,ev)bst_call(@channel_add_loc, iAllStudies, fList(iFile).fullpath, 1), []);
                        end
                    end
                end  
                ...
                if (length(bstNodes) == 1)
                    ...
                    for iMod = 1:length(DisplayModReg)
                        ...
                        if ismember(DisplayModReg{iMod}, {'MEG', 'EEG', 'NIRS'})
                            gui_component('MenuItem', jMenuAlign, [], [strType 'Check'], IconLoader.ICON_ALIGN_CHANNELS, [], @(h,ev)bst_call(@channel_align_manual, filenameRelative, DisplayModReg{iMod}, 0), []);
                            if ~bst_get('ReadOnly')
                                gui_component('MenuItem', jMenuAlign, [], [strType 'Edit...'], IconLoader.ICON_ALIGN_CHANNELS, [], @(h,ev)bst_call(@channel_align_manual, filenameRelative, DisplayModReg{iMod}, 1), []);
                            end
                        ...
                        elseif ismember(DisplayModReg{iMod}, {'SEEG', 'ECOG'}) && ~bst_get('ReadOnly')
                            ...
                            if ismember(DisplayModReg{iMod}, DisplayMod)
                                if ~isempty(sSubject.iScalp)
                                    gui_component('MenuItem', jMenuAlign, [], [strType 'Edit...    (Head)'],       IconLoader.ICON_ALIGN_CHANNELS, [], @(h,ev)bst_call(@channel_align_manual, filenameRelative, DisplayModReg{iMod}, 1, 'scalp'), []);
                                end
                                if ~isempty(sSubject.iInnerSkull)
                                    gui_component('MenuItem', jMenuAlign, [], [strType 'Edit...    (Inner skull)'],IconLoader.ICON_ALIGN_CHANNELS, [], @(h,ev)bst_call(@channel_align_manual, filenameRelative, DisplayModReg{iMod}, 1, 'innerskull'), []);
                                end
                                if ~isempty(sSubject.iCortex)
                                    gui_component('MenuItem', jMenuAlign, [], [strType 'Edit...    (Cortex)'],     IconLoader.ICON_ALIGN_CHANNELS, [], @(h,ev)bst_call(@channel_align_manual, filenameRelative, DisplayModReg{iMod}, 1, 'cortex'), []);
                                end
                                if ~isempty(sSubject.iAnatomy)
                                    gui_component('MenuItem', jMenuAlign, [], [strType 'Edit...    (MRI 3D)'],     IconLoader.ICON_ALIGN_CHANNELS, [], @(h,ev)bst_call(@channel_align_manual, filenameRelative, DisplayModReg{iMod}, 1, 'anatomy'), []);
                                end
                            end
                            % Allow edition in MRI even if there is not location available for any electrode
                            if ~isempty(sSubject.iAnatomy)
                                gui_component('MenuItem', jMenuAlign, [], [strType 'Edit...    (MRI Viewer)'], IconLoader.ICON_ALIGN_CHANNELS, [], @(h,ev)bst_call(@DisplayChannelsMri, filenameRelative, DisplayModReg{iMod}), []);
                            end
                            AddSeparator(jMenuAlign);
                        end
                    end
                    ...
                    % === LOAD SSP PROJECTORS ===
                    if ~bst_get('ReadOnly')
                        gui_component('MenuItem', jPopup, [], 'Load SSP projectors', IconLoader.ICON_CONDITION, [], @(h,ev)bst_call(@LoadSSP, filenameFull), []);
                    end
                    % === COMPUTE HEAD MODEL ===
                    if ~bst_get('ReadOnly')
                        fcnPopupComputeHeadmodel();
            ...
            case 'anatomy'
                ...
                jMenuDisplay = gui_component('Menu', jPopup, [], 'Display', IconLoader.ICON_ANATOMY, [], [], []);
                    ...
                    if ~bstNodes(1).isMarked()
                        ...
                        MriFile = sSubject.Anatomy(sSubject.iAnatomy).FileName;
                        % Overlay menus
                        gui_component('MenuItem', jMenuDisplay, [], 'Overlay on default MRI (MRI Viewer)', IconLoader.ICON_ANATOMY, [], @(h,ev)bst_call(@view_mri, MriFile, filenameRelative), []);
                        gui_component('MenuItem', jMenuDisplay, [], 'Overlay on default MRI (3D)',         IconLoader.ICON_ANATOMY, [], @(h,ev)bst_call(@view_mri_3d, MriFile, filenameRelative));
                        AddSeparator(jMenuDisplay);
                    end
                ...
                if ~bst_get('ReadOnly')
                    AddSeparator(jPopup);
                    gui_component('MenuItem', jPopup, [], 'Compute MNI transformation',    IconLoader.ICON_ANATOMY, [], @(h,ev)bst_call(@NormalizeMri, iSubject), []);
                    AddSeparator(jPopup);
                ...
                if (length(bstNodes) >= 2)
                    ...
                    jMenuAlign = gui_component('Menu', jPopup, [], 'MRI registration', IconLoader.ICON_ALIGN_SURFACES, [], [], []);
                        ...
                        if ~bst_get('ReadOnly')
                            ...
                            gui_component('MenuItem', jMenuAlign, [], 'Load BrainSuite square...', IconLoader.ICON_FOLDER_OPEN, [], @(h,ev)bst_call(@TessAddSquare, filenameRelative), []);
                            gui_component('MenuItem', jMenuAlign, [], 'Display registration sphere/square', IconLoader.ICON_DISPLAY,     [], @(h,ev)bst_call(@view_surface_sphere, filenameRelative), []);
                        end
            ...
            case {'noisecov', 'ndatacov'}
                if (length(bstNodes) == 1)
                    ...
                    if ~bst_get('ReadOnly')
                        ...
                        AddSeparator(jPopup);
                        gui_component('MenuItem', jPopup, [], 'Copy to other folders', IconLoader.ICON_HEADMODEL, [], @(h,ev)bst_call(@db_set_noisecov, bstNodes(1).getStudyIndex(), 'AllConditions', isDataCov), []);
                        gui_component('MenuItem', jPopup, [], 'Copy to other subjects',   IconLoader.ICON_HEADMODEL, [], @(h,ev)bst_call(@db_set_noisecov, bstNodes(1).getStudyIndex(), 'AllSubjects', isDataCov), []);
                    end
                end
...
%% ===== POPUP: HEADMODEL =====
            case 'headmodel'
                ...
                % === COMPUTE SOURCES ===
                if ~bst_get('ReadOnly')
                    gui_component('MenuItem', jPopup, [], 'Compute sources', IconLoader.ICON_RESULTS, [], @(h,ev)bst_call(@selectHeadmodelAndComputeSources, bstNodes, 0), []);
                    gui_component('MenuItem', jPopup, [], 'Compute sources [2015]', IconLoader.ICON_RESULTS, [], @(h,ev)bst_call(@selectHeadmodelAndComputeSources, bstNodes, 1), []);
                end
                ...
                if strcmpi(sStudy.HeadModel(iHeadModel).HeadModelType, 'volume') 
                    ...
                    gui_component('MenuItem', jPopup, [], 'Check source grid', IconLoader.ICON_HEADMODEL, [], @(h,ev)bst_call(@view_gridloc, filenameFull), []);
                elseif strcmpi(sStudy.HeadModel(iHeadModel).HeadModelType, 'mixed')
                    if ~bst_get('ReadOnly') && ~isSepGain
                        AddSeparator(jPopup);
                    end
                    gui_component('MenuItem', jPopup, [], 'Check source grid (volume)', IconLoader.ICON_HEADMODEL, [], @(h,ev)bst_call(@view_gridloc, filenameFull, 'V'), []);
                    gui_component('MenuItem', jPopup, [], 'Check source grid (surface)', IconLoader.ICON_HEADMODEL, [], @(h,ev)bst_call(@view_gridloc, filenameFull, 'S'), []);
                end
                ...
                if (length(bstNodes) == 1)
                    ...
                    if ~isempty(AllMod)
                        ...
                        for iMod = 1:length(AllMod)
                            ...
                            gui_component('MenuItem', jMenuModality, [], 'Display as image', IconLoader.ICON_NOISECOV, [], @(h,ev)bst_call(@view_erpimage, filenameRelative, AllMod{iMod}, 'trialimage'), []);
                            % == DISPLAY TOPOGRAPHY ==
                        ...
                        if ~bst_get('ReadOnly')
                            ...
                            % EDIT GOOD/BAD
                            gui_component('MenuItem', jPopupMenuGoodBad, [], 'Edit good/bad channels...', IconLoader.ICON_GOODBAD, [], @(h,ev)bst_call(@gui_edit_channelflag, filenameRelative), []);
                            % === GOOD/BAD TRIAL ===
                    ...
                    % === MENU: REVIEW AS RAW
                    if ~strcmpi(DataType, 'raw')
                        gui_component('MenuItem', jPopup, [], 'Review as raw', IconLoader.ICON_RAW_DATA, [], @(h,ev)bst_call(@import_raw, filenameFull, 'BST-DATA', iSubject), []);
                    end
                else
                    % === ERP IMAGE ===
                    if ~isempty(AllMod)
                        ...
                        for iMod = 1:length(AllMod)
                            channelTypeDisplay = getChannelTypeDisplay(AllMod{iMod}, AllMod);
                            gui_component('MenuItem', jMenuErp, [], channelTypeDisplay, IconLoader.ICON_NOISECOV, [], @(h,ev)bst_call(@view_erpimage, GetAllFilenames(bstNodes), AllMod{iMod}, 'erpimage'), []);
                        end
                        AddSeparator(jPopup);
                    end
                ...
                if (length(bstNodes) == 1)
                    ...
                    if ~isempty(AllMod)
                        ...
                        for iMod = 1:length(AllMod)
                            ...
                            gui_component('MenuItem', jMenuModality, [], 'Display as image', IconLoader.ICON_NOISECOV, [], @(h,ev)bst_call(@view_erpimage, filenameRelative, AllMod{iMod}), []);
                            % == DISPLAY TOPOGRAPHY ==
                        ...
                        if ~isempty(strfind(filenameRelative, '_cluster'))
                            ...
                            gui_component('MenuItem', jMenuCluster, [], 'Cluster indices',   IconLoader.ICON_TIMEFREQ, [], @(h,ev)bst_call(@view_statcluster, filenameRelative, 'clustindex_time', []), []);
                            gui_component('MenuItem', jMenuCluster, [], 'Cluster size', IconLoader.ICON_TS_DISPLAY, [], @(h,ev)bst_call(@view_statcluster, filenameRelative, 'clustsize_time', []), []);
                            % Modality menus
                            ...
                            if (length(topoMod) > 1)
                                ...
                                for iMod = 1:length(topoMod)
                                    ...
                                    gui_component('MenuItem', jMenuModality, [], channelTypeDisplay, IconLoader.ICON_TOPOGRAPHY, [], @(h,ev)bst_call(@view_statcluster, filenameRelative, 'longest', topoMod{iMod}), []);
                                end
                            elseif (length(topoMod) == 1)
                                gui_component('MenuItem', jMenuCluster, [], 'Longest significance', IconLoader.ICON_TOPOGRAPHY, [], @(h,ev)bst_call(@view_statcluster, filenameRelative, 'longest', topoMod{1}), []);
                            end
                        end
                        % === VIEW CLUSTERS ===
                        fcnPopupClusterTimeSeries();
            ...
            case 'datalist'                
                if ~bst_get('ReadOnly')
                    ...
                    if ~isempty(AllMod)
                        ...
                        for iMod = 1:length(AllMod)
                            channelTypeDisplay = getChannelTypeDisplay(AllMod{iMod}, AllMod);
                            gui_component('MenuItem', jMenuErp, [], channelTypeDisplay, IconLoader.ICON_NOISECOV, [], @(h,ev)bst_call(@view_erpimage, GetAllFilenames(bstNodes, 'data'), AllMod{iMod}, 'erpimage'), []);
                        end
                        AddSeparator(jPopup);
                    end
            ...
            case {'results', 'link'}
                ...
                if (length(bstNodes) == 1)
                    ...
                    if strcmpi(sStudy.Result(iResult).HeadModelType, 'surface') && ~isempty(sSubject) && ~isempty(sSubject.iCortex)
                        ...
                        gui_component('MenuItem', jMenuActivations, [], 'Display on spheres/squares', IconLoader.ICON_SURFACE, [], @(h,ev)bst_call(@view_surface_sphere, filenameRelative), []);
                    end
                end
                ...
                if ~bst_get('ReadOnly') && ~isRaw && strcmpi(sStudy.Result(iResult).HeadModelType, 'surface') % && ~isStat 
            ...
            case 'presults'
                ...
                if (length(bstNodes) == 1)
                    ...
                    if ~isempty(strfind(filenameRelative, '_cluster'))
                        jMenuCluster = gui_component('Menu', jPopup, [], 'Significant clusters', IconLoader.ICON_ATLAS, [], [], []);
                        gui_component('MenuItem', jMenuCluster, [], 'Cluster size', IconLoader.ICON_TS_DISPLAY, [], @(h,ev)bst_call(@view_statcluster, filenameRelative, 'clustsize_time', []), []);
                        gui_component('MenuItem', jMenuCluster, [], 'Longest significance', IconLoader.ICON_SURFACE_CORTEX, [], @(h,ev)bst_call(@view_statcluster, filenameRelative, 'longest'), []);
                    end
                end
                % === VIEW SCOUTS ===
                fcnPopupScoutTimeSeries(jMenuActivations, 1);
...
%% ===== POPUP: DIPOLES =====
            case 'dipoles'
                ...
                if (length(bstNodes) == 1)
                    ...
                    gui_component('MenuItem', jPopup, [], 'Merge dipoles', IconLoader.ICON_DIPOLES, [], @(h,ev)bst_call(@dipoles_merge, GetAllFilenames(bstNodes)), []);
                end
            ...
            case {'timefreq', 'ptimefreq'}
                ...
                if (length(bstNodes) == 1)
                    ...
                    if ~isempty(strfind(filenameRelative, '_connectn')) || ~isempty(strfind(filenameRelative, '_connect1'))
                        % [NxN] only
                        if ~isempty(strfind(filenameRelative, '_connectn'))                           
                            gui_component('MenuItem', jPopup, [], 'Display as graph     [NxN]',   IconLoader.ICON_CONNECTN, [], @(h,ev)bst_call(@view_connect, filenameRelative, 'GraphFull'), []);
                            ...
                            gui_component('MenuItem', jPopup, [], 'Display as image    [NxN]', IconLoader.ICON_NOISECOV, [], @(h,ev)bst_call(@view_connect, filenameRelative, 'Image'), []);
                        ...
                        switch lower(DataType)
                            case 'data'
                                ...
                                if ~isempty(strfind(filenameRelative, '_plvt')) || ~isempty(strfind(filenameRelative, '_corr_time')) || ~isempty(strfind(filenameRelative, '_cohere_time'))
                                    ...
                                    gui_component('MenuItem', jMenuConn1, [], 'One row',     IconLoader.ICON_TIMEFREQ, [], @(h,ev)bst_call(@view_timefreq, filenameFull, 'SingleSensor'), []);
                                    gui_component('MenuItem', jMenuConn1, [], 'All rows',    IconLoader.ICON_TIMEFREQ, [], @(h,ev)bst_call(@view_timefreq, filenameFull, 'AllSensors'), []);
                                end
                            ...
                            case 'results'
                                % One channel
                                if ~isempty(strfind(filenameRelative, '_plvt')) || ~isempty(strfind(filenameRelative, '_corr_time')) || ~isempty(strfind(filenameRelative, '_cohere_time'))
                                    gui_component('MenuItem', jMenuConn1, [], 'One channel', IconLoader.ICON_TIMEFREQ, [], @(h,ev)bst_call(@view_timefreq, filenameFull, 'SingleSensor'), []);
                                    AddSeparator(jMenuConn1);
                                end
                                % Cortex
                                if isempty(strfind(filenameRelative, '_connectn')) && ~isempty(sSubject) && ~isempty(sSubject.iCortex) && ~isVolume
                                    gui_component('MenuItem', jMenuConn1, [], 'Display on cortex', IconLoader.ICON_CORTEX, [], @(h,ev)bst_call(@view_surface_data, [], filenameRelative), []);
                                end
                                % MRI
                                if isempty(strfind(filenameRelative, '_connectn')) && ~isempty(sSubject) && ~isempty(sSubject.iAnatomy)
                                    MriFile = sSubject.Anatomy(sSubject.iAnatomy).FileName;
                                    gui_component('MenuItem', jMenuConn1, [], 'Display on MRI   (3D)',         IconLoader.ICON_ANATOMY, [], @(h,ev)bst_call(@view_surface_data, MriFile, filenameRelative), []);
                                    gui_component('MenuItem', jMenuConn1, [], 'Display on MRI   (MRI Viewer)', IconLoader.ICON_ANATOMY, [], @(h,ev)bst_call(@view_mri, MriFile, filenameRelative), []);
                                end
                            otherwise
                                if ~isempty(strfind(filenameRelative, '_cohere')) || ~isempty(strfind(filenameRelative, '_spgranger')) || ~isempty(strfind(filenameRelative, '_plv')) || ~isempty(strfind(filenameRelative, '_plvt'))
                                    gui_component('MenuItem', jPopup, [], 'Power spectrum', IconLoader.ICON_SPECTRUM, [], @(h,ev)bst_call(@view_spectrum, filenameRelative, 'Spectrum'), []);
                                ...
                                if isempty(strfind(filenameRelative, '_connectn'))
                                    gui_component('MenuItem', jMenuConn1, [], 'Display as image', IconLoader.ICON_NOISECOV, [], @(h,ev)bst_call(@view_connect, filenameRelative, 'Image'), []);
                                end
                        end
                    ...
                    % ===== TIME-FREQUENCY =====
                        ...
                        switch lower(DataType)
                            case 'data'
                                ...
                                gui_component('MenuItem', jPopup, [], 'Power spectrum', IconLoader.ICON_SPECTRUM, [], @(h,ev)bst_call(@view_spectrum, filenameRelative, 'Spectrum'), []);
                                gui_component('MenuItem', jPopup, [], 'Time series',    IconLoader.ICON_DATA,     [], @(h,ev)bst_call(@view_spectrum, filenameRelative, 'TimeSeries'), []);
                                gui_component('MenuItem', jPopup, [], 'Image [channels x time]', IconLoader.ICON_NOISECOV, [], @(h,ev)bst_call(@view_erpimage, filenameRelative, [], 'trialimage'), []);
                                AddSeparator(jPopup);
                                % Topography
                                if ~isempty(DisplayMod)
                            ...
                            otherwise
                                ...
                                if isempty(strfind(filenameRelative, '_pac'))
                                    gui_component('MenuItem', jPopup, [], 'Power spectrum', IconLoader.ICON_SPECTRUM, [], @(h,ev)bst_call(@view_spectrum, filenameRelative, 'Spectrum'), []);
                                    gui_component('MenuItem', jPopup, [], 'Time series',    IconLoader.ICON_DATA,     [], @(h,ev)bst_call(@view_spectrum, filenameRelative, 'TimeSeries'), []);
                                    gui_component('MenuItem', jPopup, [], 'Image [signals x time]', IconLoader.ICON_NOISECOV, [], @(h,ev)bst_call(@view_erpimage, filenameRelative, [], 'trialimage'), []);
                                end
                        end
                        % Export to file
...
%% ===== POPUP: VIDEO =====
            case 'video'
                if (length(bstNodes) == 1)
                    if ispc
                        gui_component('MenuItem', jPopup, [], 'Windows Media Player', IconLoader.ICON_VIDEO, [], @(h,ev)bst_call(@view_video, filenameFull, 'WMPlayer', 1), []);
                    end
                    gui_component('MenuItem', jPopup, [], 'MATLAB VideoReader', IconLoader.ICON_VIDEO, [], @(h,ev)bst_call(@view_video, filenameFull, 'VideoReader', 1), []);
                end
...
%% ===== POPUP: COMMON MENUS =====
        ...
        jMenuFile = gui_component('Menu', [], [], 'File', IconLoader.ICON_MATLAB, [], [], []);
            ...
            % ===== VIEW HISTOGRAM =====
            if isfile && ~ismember(nodeType, {'subject', 'study', 'studysubject', 'condition', 'rawcondition', 'datalist', 'matrixlist', 'image', 'channel', 'rawdata', 'dipoles'})
                gui_component('MenuItem', jMenuFile, [], 'View histogram', IconLoader.ICON_HISTOGRAM, [], @(h,ev)bst_call(@view_histogram, GetAllFilenames(bstNodes)), []);
            end
            ...
            if ~bst_get('ReadOnly') && isone && isMatlabRunning
                if isstudy
                    ...
                    gui_component('MenuItem', jMenuFile, [], 'Import data matrix', IconLoader.ICON_MATRIX, [], @(h,ev)bst_call(@import_matrix, iStudy), []);
                elseif issubject
            ...
            if ~bst_get('ReadOnly') && isone && isstudy && strcmpi(nodeType, 'rawcondition')
                AddSeparator(jMenuFile);
                gui_component('MenuItem', jMenuFile, [], 'Delete raw file', IconLoader.ICON_RAW_DATA, [], @(h,ev)import_video(iStudy), []);
            end
            if ~bst_get('ReadOnly') && isone && ((isstudy && strcmpi(nodeType, 'rawcondition')) ||  strcmpi(nodeType, 'rawdata'))
            ...
            if ~bst_get('ReadOnly') && isone && ~isSpecialNode
                ...
                elseif strcmpi(nodeType, 'condition') && (bstNodes(1).getStudyIndex ~= 0)
                    gui_component('MenuItem', jMenuFile, [], 'Duplicate folder', IconLoader.ICON_COPY, [], @(h,ev)bst_call(@process_duplicate, 'DuplicateCondition', filenameRelative, '_copy'), []);
                end
            end
            % ===== SEND TO PROCESS ======
...
%% ===== MENU: NOISE COV =====
    function fncPopupMenuNoiseCov()
        ...
        gui_component('MenuItem', jMenu, [], 'Import from file',   IconLoader.ICON_NOISECOV, [], @(h,ev)bst_call(@tree_set_noisecov, bstNodes, [], 0), []);
        gui_component('MenuItem', jMenu, [], 'Import from Matlab', IconLoader.ICON_NOISECOV, [], @(h,ev)bst_call(@tree_set_noisecov, bstNodes, 'MatlabVar', 0), []);
        AddSeparator(jMenu);
        gui_component('MenuItem', jMenu, [], 'Compute from recordings', IconLoader.ICON_NOISECOV, [], @(h,ev)bst_call(@tree_set_noisecov, bstNodes, 'Compute', 0), []);
        AddSeparator(jMenu);
        gui_component('MenuItem', jMenu, [], 'No noise modeling (identity matrix)', IconLoader.ICON_NOISECOV, [], @(h,ev)bst_call(@tree_set_noisecov, bstNodes, 'Identity', 0), []);
        % Data covariance
        jMenu = gui_component('Menu', jPopup, [], 'Data covariance', IconLoader.ICON_NOISECOV, [], [], []);
        gui_component('MenuItem', jMenu, [], 'Import from file',   IconLoader.ICON_NOISECOV, [], @(h,ev)bst_call(@tree_set_noisecov, bstNodes, 1), []);
        gui_component('MenuItem', jMenu, [], 'Import from Matlab', IconLoader.ICON_NOISECOV, [], @(h,ev)bst_call(@tree_set_noisecov, bstNodes, 'MatlabVar', 1), []);
        AddSeparator(jMenu);
        gui_component('MenuItem', jMenu, [], 'Compute from recordings', IconLoader.ICON_NOISECOV, [], @(h,ev)bst_call(@tree_set_noisecov, bstNodes, 'Compute', 1), []);
    end

%% ===== MENU: COMPUTE SOURCES =====
    function fcnPopupComputeSources()         
        import org.brainstorm.icon.*;
        gui_component('MenuItem', jPopup, [], 'Compute sources', IconLoader.ICON_RESULTS, [], @(h,ev)bst_call(@panel_protocols, 'TreeInverse', bstNodes, 0), []);
        gui_component('MenuItem', jPopup, [], 'Compute sources [2015]', IconLoader.ICON_RESULTS, [], @(h,ev)bst_call(@panel_protocols, 'TreeInverse', bstNodes, 1), []);
    end
...
function fcnPopupTopoNoInterp(jMenu, FileName, AllMod, is2DLayout, isGradNorm, AlwaysCreate)
    ...
    for iMod = 1:length(AllMod)
        ...
        gui_component('MenuItem', jSubMenu, [], '3D Sensor cap', IconLoader.ICON_TOPOGRAPHY, [], @(h,ev)view_topography(FileName, AllMod{iMod}, '3DSensorCap', [], UseMagneticExtrap, hFig), []);
        gui_component('MenuItem', jSubMenu, [], '2D Sensor cap', IconLoader.ICON_TOPOGRAPHY, [], @(h,ev)view_topography(FileName, AllMod{iMod}, '2DSensorCap', [], UseMagneticExtrap, hFig), []);
        gui_component('MenuItem', jSubMenu, [], '2D Disc',       IconLoader.ICON_TOPOGRAPHY, [], @(h,ev)view_topography(FileName, AllMod{iMod}, '2DDisc',      [], UseMagneticExtrap, hFig), []);
        % 2D Layout
        if is2DLayout
            gui_component('MenuItem', jSubMenu, [], '2D Layout', IconLoader.ICON_2DLAYOUT, [], @(h,ev)view_topography(FileName, AllMod{iMod}, '2DLayout'), []);
        end
        % 3D Electrodes
        if ismember(AllMod{iMod}, {'EEG', 'ECOG'})
...
%% ===== DISPLAY CHANNELS (3D) =====
function [hFig, iDS, iFig] = DisplayChannels(bstNodes, Modality, SurfaceType, is3DElectrodes)
    % Parse inputs
    ...
    if ~isempty(sSubject)
        ...
        if ismember(Modality, {'Vectorview306', 'CTF', '4D', 'KIT', 'KRISS', 'BabyMEG'})
            opaqueAlpha = 0;
        else
            opaqueAlpha = .1;
        end
    ...
    if (length(FileNames) == 1)
        ...
        [hFig, iDS, iFig] = view_channels(FileNames{1}, Modality, 1, isLabels, hFig, is3DElectrodes);
...
%% ===== NORMALIZE MRI =====
function NormalizeMri(iSubject)
    % Unloading everything
    bst_memory('UnloadAll', 'Forced');
    % Call normalize function
    [sMri, errMsg] = bst_normalize_mni(iSubject);
    % Error handling
    if ~isempty(errMsg)
