...
% STR_FORMAT: Format anytime of data, either in a human readable way, or for creating reusable Matlab scripts.
%
    ...
    if isCode
        ...
        for iElem = 1:numel(s)
            ...
            for iField = 1:length(MatFields)
                ...
                structString = [structString, indentStr2, '''', MatFields{iField}, ''', ' strField];
    ...
    % Human-readable text file
        ...
        % More than one field
            ...
            if (M*N == 1)
                ...
                % Get the size of the longest fieldname
                maxFieldName = 0;
                for iField = 1:length(MatFields)
                    maxFieldName = max(maxFieldName, length(MatFields{iField}));
                end
                ...
                for iField = 1:length(MatFields)
                    ...
                    tabulations = repmat(' ', [1, maxFieldName - length(MatFields{iField})]);
