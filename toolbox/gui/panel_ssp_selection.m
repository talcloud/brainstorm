...
%% ===== PLOT COMPONENTS =====
function PlotComponents(UseMagneticExtrap, isPlotTopo, isPlotTs)
    global GlobalData EditSspPanel;
    % Get current dataset
...
%     % Time series: only for SSP/PCA and ICA
%     if isPlotTs && (isempty(sCat.SingVal) || isequal(sCat.SingVal, 'REF'))
%         disp('BST> Cannot display time series for this projector.');
%         return;
%     end
    ...
    allMod = unique({GlobalData.DataSet(iDS).Channel(any(sCat.Components,2)).Type});
...
%     end
    ...
    for iMod = 1:length(allMod)
        ...
        if isPlotTopo
            ...
            if (length(iComp) == 1)
                EditSspPanel.hFigTopo(end+1) = view_topography(DataFile, allMod{iMod}, [], Topo, UseMagneticExtrap, 'NewFigure');
            % Plot all the components in a contact sheet
            else
                % Open figure
                hFig = view_topography(DataFile, allMod{iMod}, [], zeros(length(iChannels),1), UseMagneticExtrap, 'NewFigure');
                if isempty(hFig)
                    return;
                end
