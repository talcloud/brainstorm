...
function GUI = CreateWindow() %#ok<DEFNU>
    ...
    jMenuFile = gui_component('Menu', jMenuBar, [], 'File', [], [], [], menuSize);
        ...
        gui_component('MenuItem', jMenuFile, [], 'Export protocol',     IconLoader.ICON_SAVE, [], @(h,ev)bst_call(@export_protocol), []);
...
%         jMenuFile.addSeparator();
        % === PROCESSES ===
        gui_component('MenuItem', jMenuFile, [], 'Report viewer',        IconLoader.ICON_EDIT,    [], @(h,ev)bst_report('Open', 'current'), []);
        gui_component('MenuItem', jMenuFile, [], 'Reload last pipeline', IconLoader.ICON_PROCESS, [], @(h,ev)bst_report('Recall', 'current'), []);
        % === SET PREFERENCES ===
        gui_component('MenuItem', jMenuFile, [], 'Edit preferences', IconLoader.ICON_PROPERTIES, [], @(h,ev)gui_show('panel_options', 'JavaWindow', 'Brainstorm preferences', [], 1, 0, 0), []);
        jMenuFile.addSeparator();
        % === DIGITIZE ===
        ...
        gui_component('MenuItem', jMenuFile, [], 'Digitize', IconLoader.ICON_CHANNEL, [], @(h,ev)panel_digitize('Start'), []);
        jMenuFile.addSeparator();
        % === QUIT ===
    ...
    jMenuSupport = gui_component('Menu', jMenuBar, [], 'Help', [], [], [], menuSize);
        ...
        jMenuOpenmeeg = gui_component('Menu', jMenuSupport, [], 'Update OpenMEEG', [], [], [], []);
            gui_component('MenuItem', jMenuOpenmeeg, [], 'Download', [], [], @(h,ev)DownloadOpenmeeg(), []);
            gui_component('MenuItem', jMenuOpenmeeg, [], 'Install', [], [], @(h,ev)bst_openmeeg('update'), []);
    ...
    jToolbarA.setOrientation(javax.swing.JToolBar.VERTICAL);
        ...
        % Buttons
        jButtonRecordingsA = gui_component('toolbartoggle', jToolbarA, [], '', {IconLoader.ICON_DATA_LIST,     TB_SIZE, jButtonGroupType}, 'Process recordings',  @(h,ev)ProcessDataType_Callback('data', 'A', ev));
        jButtonSourcesA    = gui_component('toolbartoggle', jToolbarA, [], '', {IconLoader.ICON_RESULTS_LIST,  TB_SIZE, jButtonGroupType}, 'Process sources',     @(h,ev)ProcessDataType_Callback('results', 'A', ev));
        jButtonTimefreqA   = gui_component('toolbartoggle', jToolbarA, [], '', {IconLoader.ICON_TIMEFREQ_LIST, TB_SIZE, jButtonGroupType}, 'Process time-freq',   @(h,ev)ProcessDataType_Callback('timefreq', 'A', ev));
        jButtonMatrixA     = gui_component('toolbartoggle', jToolbarA, [], '', {IconLoader.ICON_MATRIX_LIST,   TB_SIZE, jButtonGroupType}, 'Process user matrix', @(h,ev)ProcessDataType_Callback('matrix', 'A', ev));
    ...
    jToolbarB.setOrientation(javax.swing.JToolBar.VERTICAL);
        jButtonGroupType = java_create('javax.swing.ButtonGroup');
        % Buttons
        jButtonRecordingsB = gui_component('toolbartoggle', jToolbarB, [], '', {IconLoader.ICON_DATA_LIST,     TB_SIZE, jButtonGroupType}, 'Process recordings',  @(h,ev)ProcessDataType_Callback('data', 'B', ev));
        jButtonSourcesB    = gui_component('toolbartoggle', jToolbarB, [], '', {IconLoader.ICON_RESULTS_LIST,  TB_SIZE, jButtonGroupType}, 'Process sources',     @(h,ev)ProcessDataType_Callback('results', 'B', ev));
        jButtonTimefreqB   = gui_component('toolbartoggle', jToolbarB, [], '', {IconLoader.ICON_TIMEFREQ_LIST, TB_SIZE, jButtonGroupType}, 'Process time-freq',   @(h,ev)ProcessDataType_Callback('timefreq', 'B', ev));
        jButtonMatrixB     = gui_component('toolbartoggle', jToolbarB, [], '', {IconLoader.ICON_MATRIX_LIST,   TB_SIZE, jButtonGroupType}, 'Process user matrix', @(h,ev)ProcessDataType_Callback('matrix', 'B', ev));
...
%% ===== PROTOCOL CHANGED =====
    ...
    function ShowTabMenu()
        ...
        % List possible tabs
        panelList = {'Record', 'Filter', 'Surface', 'Scout', 'Cluster', 'Coordinates', 'Command'};
        panelRemove = {};
        % List missing tabs
...
function SetProcessFileType(fileType, nodelistName) %#ok<DEFNU>
    ...
    if (nargin < 2) || any(strcmpi(nodelistName, {'Process1', 'Process2A'}))
        switch lower(fileType)
            case 'data',      GUI.jButtonRecordingsA.setSelected(1);
            case 'results',   GUI.jButtonSourcesA.setSelected(1);
            case 'timefreq',  GUI.jButtonTimefreqA.setSelected(1);
            case 'matrix',    GUI.jButtonMatrixA.setSelected(1);
        end
    elseif strcmpi(nodelistName, 'Process2B')
        switch lower(fileType)
            case 'data',      GUI.jButtonRecordingsB.setSelected(1);
            case 'results',   GUI.jButtonSourcesB.setSelected(1);
            case 'timefreq',  GUI.jButtonTimefreqB.setSelected(1);
            case 'matrix',    GUI.jButtonMatrixB.setSelected(1);
        end
    end
...
%     % Test address
%     srcUrl  = 'http://neuroimage.usc.edu/brainstorm3_register/getupdate.php?t=Infant7w_2015';
%     % Get URL
