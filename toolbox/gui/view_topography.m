function [hFig, iDS, iFig] = view_topography(DataFile, Modality, TopoType, F, UseMagneticExtrap, hFig, RefRowName)
% VIEW_TOPOGRAPHY: Display MEG/EEG topography in a new figure.
%
% USAGE:  [hFig, iDS, iFig] = view_topography(DataFile, Modality, TopoType, F, UseMagneticExtrap, hFig, RefRowName)
% USAGE:  [hFig, iDS, iFig] = view_topography(DataFile, Modality, TopoType, F, UseMagneticExtrap, 'NewFigure', RefRowName)
...
%     - TopoType   : {'3DSensorCap', '2DDisc', '2DSensorCap', 2DLayout', '3DElectrodes', '3DElectrodes-Cortex', '3DElectrodes-Head', '3DElectrodes-MRI'}
...
switch(fileType)
    case 'data'
        ...
        if isempty(UseMagneticExtrap)
            UseMagneticExtrap = ismember(GlobalData.DataSet(iDS).Measures.DataType, {'recordings', 'raw'}) && ...
                                ~isempty(Modality) && ismember(Modality, {'MEG', 'MEG GRAD', 'MEG MAG'}) && ...
...
if ~strcmpi(FileType, 'Timefreq')
    ...
    if ~isempty(sMontage) && ~strcmpi(sMontage.Type, 'selection') && ~isempty(sFigMontages) && any(strcmpi({sFigMontages.Name}, sMontage.Name))
...
if isNewFig && ismember(TopoType, {'3DSensorCap', '3DElectrodes'})
...
if strcmpi(TopoType, '3DSensorCap') || strcmpi(TopoType, '3DElectrodes')
