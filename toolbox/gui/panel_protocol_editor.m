...
function bstPanelNew = CreatePanel(action) %#ok<DEFNU>
    ...
    if strcmpi(action, 'create') || strcmpi(action, 'edit')
        ...
        jPanelDefaults.add('p', JLabel('<HTML><B>Default channel file</B>: '));
        jButtonGroupChannel = ButtonGroup();
        ctrl.jRadioChannelIndividual     = JRadioButton('No, use one channel file per acquisition run (MEG).');
        ctrl.jRadioChannelDefaultSubject = JRadioButton('Yes, use one channel file per subject (EEG).');
