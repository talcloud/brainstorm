    ...
    for k = 1:length(sScouts)
        ...
        if (nComponents == 0)
            ...
            if ~isVolumeAtlas && strcmpi(GridAtlas.Scouts(iRegionScouts).Region(2), 'V')
                bst_error(['Scout "' sScouts(k).Label '" is a volume scout but region "' GridAtlas.Scouts(iRegionScouts).Label '" is a volume region.'], 'Display scouts', 0);
                return;
            elseif isVolumeAtlas && strcmpi(GridAtlas.Scouts(iRegionScouts).Region(2), 'S')
                bst_error(['Scout "' sScouts(k).Label '" is a surface scout but region "' GridAtlas.Scouts(iRegionScouts).Label '" is a surface region.'], 'Display scouts', 0);
                return;
            end
        ...
        if (nComponents == 1)
            isFlipSign = ~isStat && ~isTimefreq && ...
                         ...
                         isempty(strfind(ResultsFiles{iResFile}, '_zscore')) && ...
