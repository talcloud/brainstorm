...
function FigureMouseWheelCallback(hFig, event)
    ...
    elseif ismember('shift', get(hFig,'CurrentModifier'))
...
function UpdateFigurePlot(hFig, isForced)
    ...
    PlotHandles = PlotAxes(hFig, X, XLim, TF, TfInfo, TsInfo, sFig.Handles.DataMinMax, LinesLabels);
...
%% ===== PLOT AXES =====
function PlotHandles = PlotAxes(hFig, X, XLim, TF, TfInfo, TsInfo, DataMinMax, LinesLabels)
    %global GlobalData;
...
function PlotHandles = PlotAxesButterfly(hAxes, PlotHandles, TfInfo, TsInfo, X, TF, LinesLabels)
    ...
    if ~isempty(LinesLabels) && TsInfo.ShowLegend && ((length(LinesLabels) > 1) || ~isempty(LinesLabels{1})) % && (length(LinesLabels) <= 30) 
...
%     end
    ...
    if ~isempty(LinesLabels) && ~isempty(iDispChan)
        ...
        set(hAxes, 'YTickMode',      'manual', ...
                   ...
                   'YTick',          bst_flip(PlotHandles.ChannelOffsets(iDispChan), 1), ...
