...
function UpdatePanel(hFig)
    ...
    isEeg = ~isempty(GlobalData.DataSet(iDS).Figure(iFig).Id.Modality) && ismember(GlobalData.DataSet(iDS).Figure(iFig).Id.Modality, {'EEG','SEEG','ECOG'});
...
%% ===== READ RAW BLOCK =====
function [F, TimeVector, smpBlock] = ReadRawBlock(sFile, ChannelMat, iEpoch, TimeRange, DisplayMessages, UseCtfComp, RemoveBaseline) %#ok<DEFNU>
...
function EventConvertToExtended()
    ...
    % Apply modificiation to each event type
    for i = 1:length(sEvents)
        sEvents(i).samples = [sEvents(i).samples(1,:) + evtWindow(1); ...
...
function DeleteRawFile(DataFile, isForce) %#ok<DEFNU>
    ...
    % If it is a condition name: get the data file
    if ~strcmpi(DataFile(end-3:end), '.mat')
        % Get the study in the database
        ConditionPath = DataFile;
        [sStudy, iStudy] = bst_get('StudyWithCondition', ConditionPath);
        ...
        % Get raw link file
        DataFile = sStudy.Data(iDataRaw).FileName;
    else
        % Get condition
