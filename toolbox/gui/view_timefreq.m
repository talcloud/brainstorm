...
if ~isempty(RowName) && ischar(RowName) && ~ismember(RowName, AllRows)
    bst_error(['There is no entry "' RowName '" in this file.'], 'View timefreq', 0);
    return;
end
...
if isempty(hFig)
    bst_error('Cannot create figure', 'View timefreq', 0);
    return;
end
...
elseif ~isempty(strfind(lower(TimefreqFile), 'zscore'))
    ColormapType = 'stat1';
elseif strcmpi(file_gettype(TimefreqFile), 'ptimefreq')
    ColormapType = 'stat2';
else
