...
function UpdateFigurePlot(hFig, isForced)
    ...
    if TfInfo.HideEdgeEffects
        % If the options of the wavelets were saved in the file
        tfOptions = GlobalData.DataSet(iDS).Timefreq(iTf).Options;
        if ~isempty(tfOptions) && isfield(tfOptions, 'MorletFc') && isfield(tfOptions, 'MorletFwhmTc')
            fc = tfOptions.MorletFc;
            FWHM_tc = tfOptions.MorletFwhmTc;
            % Get time vector
            if ~iscell(Time)
                t = Time;
            else
            ...
            if ~iscell(Freqs)
                ...
                f = mean(2, process_tf_bands('GetBounds', Freqs))';
            end
            % Get time/frenquency resolution
            sigma_tc = FWHM_tc / sqrt(8*log(2));
            sigma_t = sigma_tc * fc ./ f; %standard deviation in time
            % Build TF mask
            FWHM_t = sqrt(8*log(2))*sigma_t;
            TF_timeres = repmat(FWHM_t' ./ 2, [1,length(t)]);
            TF_mask = (bst_bsxfun(@minus, t - t(1), TF_timeres) > 0);
            TF_mask = double(TF_mask & bst_flip(TF_mask, 2));
            clear TF_timeres;
        end
    end
    ...
    switch lower(TfInfo.DisplayMode)
        case 'singlesensor'
            ...
            if (TfInfo.HighResolution)
                PlotTimefreqSurfHigh(hAxes, Time, Freqs, TF, TF_mask);
            else
                PlotTimefreqSurf(hAxes, Time, FullTimeVector, Freqs, TF, TF_mask);
            end
            % Configure axes
        ...
        case '2dlayout'
            PlotAllSensors(hFig, RowNames, TF, TF_mask, MinMaxVal, 1);
        case '2dlayoutopt'
            PlotAllSensors(hFig, RowNames, TF, TF_mask, MinMaxVal, 2);
        case 'allsensors'
            PlotAllSensors(hFig, RowNames, TF, TF_mask, MinMaxVal, 0);
    end
...
function hSurf = PlotTimefreqSurf(hAxes, Time, FullTimeVector, Freqs, TF, TF_mask)
...
function PlotTimefreqSurfHigh(hAxes, Time, Freqs, TF, TF_mask)
    % Delete previous objects
    surfTag = 'TimefreqSurf';
    hOld = findobj(hAxes, '-depth', 1, 'tag', surfTag);
    delete(hOld);
    ...
    hSurf = surface('XData',     XData, ...
     ...
     if ~isempty(TF_mask)
         % Re-interpolate for high-resolution display: one value per pixel
         TF_maskhi = interp2(X1, Y1, TF_mask, X2, Y2, 'linear');
         % Set transparency
         set(hSurf, 'FaceAlpha', 'interp', ...
                    'AlphaData', double(abs(TF_maskhi) > 0.9));
     end
end
...
%% ===== PLOT ALL SENSORS =====
function PlotAllSensors(hFig, RowNames, TF, TF_mask, MinMaxVal, is2DLayout)
    % Find axes
    hAxes = findobj(hFig, '-depth', 1, 'tag', 'AxesTimefreq');
    cla(hAxes);
    hold(hAxes, 'on');
    ...
    % Downsample mask
    TF_mask = TF_mask(1:yRatio:end, 1:xRatio:end);
...
function [X,Y,axesSize] = GetLayoutPositions(hFig, RowNames, is2DLayout)
    ...
    if is2DLayout
        ...
        if all(markers_loc(:,3) < 0.0001)
            ...
            [Y,X] = bst_project_2d(markers_loc(:,1), markers_loc(:,2), markers_loc(:,3));
        end
        X = -X;
        % Normalize positions
