...
%        gui_layout('GetMaximumWindow')         : Location and size of the largest window in the current window manager
...
function [jMaxWindow, MaxWindow] = GetMaximumWindow()
...
function ScreenDef = GetScreenClientArea()
    ...
    [jMaxWindow, MaxWindow] = GetMaximumWindow();
    % Get list of screens
    ...
    % First screen (default): Use the MaxWindow information
    jBounds    = jScreens(1).getDefaultConfiguration().getBounds();
    ScreenSize = [jBounds.getX(), jBounds.getY(), jBounds.getWidth(), jBounds.getHeight()];
    ScreenDef(1).javaPos = jMaxWindow;
    ScreenDef(1).matlabPos  = [MaxWindow(1) + 1, ...
                                ScreenSize(4) - MaxWindow(2) - MaxWindow(4) + 1, ...
                                MaxWindow(3), ...
                                MaxWindow(4)];
    % Second screen (if available and enabled): Use ScreenSize
    isDoubleScreen = bst_get('Layout', 'DoubleScreen');
    if ~isempty(isDoubleScreen) && isDoubleScreen && (length(jScreens) > 1)
        jBounds    = jScreens(2).getDefaultConfiguration().getBounds();
        ScreenSize = [jBounds.getX(), jBounds.getY(), jBounds.getWidth(), jBounds.getHeight()];
        ScreenDef(2).javaPos = jBounds;
        ScreenDef(2).matlabPos  = [ScreenSize(1) + 1, ...
                                    ScreenSize(2) + 1, ...
                                    ScreenSize(3), ...
                                    ScreenSize(4)];
    end
    ...
    elseif (nbScreens == 1)
        ...
        if (distanceToLeft < distanceToRight)
            jBstArea = java.awt.Rectangle(...
                            ...
                            javaArea.getHeight());
            FigArea = [jBstArea.getX() + jBstArea.getWidth() + 1, ...
                       matlabArea(2), ...
                       matlabArea(1) + matlabArea(3) - jBstArea.getX() - jBstArea.getWidth() - 1, ...
                       matlabArea(4)];
        % === CONFIG 2 ===
        else
            jBstArea = java.awt.Rectangle(...
            ...
            FigArea = [matlabArea(1), ...
                       matlabArea(2), ...
                       jBstArea.getX() - javaArea.getX(), ...
                       matlabArea(4)];
        end
        jFigArea = java.awt.Rectangle(FigArea(1)-1, javaArea.getY(), FigArea(3), FigArea(4));

    % ===== TWO SCREENS =====
    elseif (nbScreens == 2)
        % If Brainstorm window is on screen 2
        if (jBstWindow.getBounds.getX() >= ScreenDef(2).javaPos.getX())
...
function TileWindows(UseWeights)
    ...
    elseif (nbScreens == 1)
        maxSizeOtherFigures = [700, 550];
    else
        maxSizeOtherFigures = [2048, 550];
    end
