...
function FigureMouseUpCallback(hFig, varargin)
    ...
    if ~hasMoved
        ...
        % === SELECTING SENSORS ===
            ...
            %if ismember(get(clickObject,'Tag'), {'ElectrodeGrid', 'ElectrodeSelect'})
...
function FigureKeyPressedCallback(hFig, keyEvent)   
    ...
    % If figure is 2D
    is2D = ~strcmpi(FigureId.Type, '3DViz') && ~ismember(FigureId.SubType, {'3DSensorCap', '3DElectrodes'});
    ...
    switch (keyEvent.Character)
        ...
        case '+'
            ...
            FigureMouseWheelCallback(hFig, event);
        case '-'
...
function ApplyViewToAllFigures(hSrcFig, isView, isSurfProp)
    ...
    for i = 1:length(hAllFig)
        ...
        if isempty(FigureId) || (strcmpi(FigureId.Type, 'Topography') && ~ismember(FigureId.SubType, {'3DElectrodes', '3DSensorCap'}))
            continue;
        end
...
function DisplayFigurePopup(hFig)
    ...
    if ~isAlignFig && ~isempty(GlobalData.DataSet(iDS).ChannelFile)
        ...
        isMarkers = ~isempty(GlobalData.DataSet(iDS).Figure(iFig).Handles.hSensorMarkers) || ...
                    ismember(GlobalData.DataSet(iDS).Figure(iFig).Id.SubType, {'2DLayout', '3DElectrodes'});
    ...
    for i = 1:length(Channel)
        ...
        switch (Device)
            ...
            case 'NIRS-BRS'
                chan_loc    = [chan_loc,    mean(Channel(i).Loc,2)];
                markers_loc = [markers_loc, mean(Channel(i).Loc,2)];
                vertices    = [vertices,    mean(Channel(i).Loc,2)];
            otherwise
                chan_loc    = [chan_loc,    mean(Channel(i).Loc,2)];
                markers_loc = [markers_loc, Channel(i).Loc(:,1)];
                vertices    = [vertices,    Channel(i).Loc];
...
function ViewSensors(hFig, isMarkers, isLabels, isMesh, Modality)
    ...
    isTopography = strcmpi(Figure.Id.Type, 'Topography') && ~ismember(Figure.Id.SubType, {'3DElectrodes'});
    is2D = 0;
    ...
    if ~isempty(isMarkers)
        ...
        if isMarkers
            ...
            elseif strcmpi(Modality, 'Vectorview306')
                ...
                isLabels = 0;
            % If CTF/4D
            elseif strcmpi(Modality, 'CTF') || strcmpi(Modality, '4D') || strcmpi(Modality, 'KIT') || strcmpi(Modality, 'KRISS') || strcmpi(Modality, 'BabyMEG')
