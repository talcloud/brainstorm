...
%
%     Parametric significance estimation:  Kuramaswamy's CDF using using Goodman's formula from [1], simplified by the null hypothesis as in [2]
%       => pValues = (1 - MSC) .^ floor(signalLength / windowLength)
%
% References:
%   [1] Carter, G. C. (1987). Coherence and time delay estimation. Proc IEEE,
%       75(2), 236?255. doi:10.1109/PROC.1987.13723
%   [2] Amjad, A. M., Halliday, D. M., Rosenberg, J. R., & Conway, B. A.
%       (1997). An extended difference of coherence test for comparing and
%       combining several independent coherence estimates: theory and
%       application to the study of motor units and physiological tremor.
...
if ~isSymmetric
    ...
    switch (CohMeasure)
        case 'mscohere'
            ...
            pValues = (1 - Gxy) .^ floor(nSamples / nFFT);
        case 'icohere'
            % Coherence function: C = Gxy/sqrt(Gxx*Gxy)
            Gxy = bst_bsxfun(@rdivide, Gxy, sqrt(Gxx));
            Gxy = bst_bsxfun(@rdivide, Gxy, sqrt(Gyy));
            % Parametric estimation of the significance level
            pValues = (1 - abs(Gxy).^2) .^ floor(nSamples / nFFT);
