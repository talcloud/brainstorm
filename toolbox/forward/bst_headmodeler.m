...
% Build default Comment : GRIDTYPE METHODMEG METHODEEG ...
    ...
    case 'volume'
        if isempty(OPTIONS.GridLoc)
            ...
            if (OPTIONS.Interactive)
                OPTIONS.GridLoc = gui_show_dialog('Volume source grid', @panel_sourcegrid, 0, [], OPTIONS.CortexFile);
            else
                OPTIONS.GridLoc = panel_sourcegrid('GetDefaultGrid', OPTIONS.CortexFile);
            end
            if isempty(OPTIONS.GridLoc)
                OPTIONS = [];
                bst_progress('stop');
