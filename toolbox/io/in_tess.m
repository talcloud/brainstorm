...
switch (FileFormat)
    ...
    case 'CURRY-BEM'
        ...
        % Convert into local MRI coordinates
%         if ~isempty(sMri)
%             TessMat.Vertices = bst_bsxfun(@rdivide, TessMat.Vertices, sMri.Voxsize) / 1000;
%         end
...
%             TessMat.Vertices = bst_bsxfun(@plus, TessMat.Vertices, mriSize / 1000);
%             TessMat.Vertices = bst_bsxfun(@times, TessMat.Vertices, sMri.Voxsize);
%         end
        % FreeSurfer RAS coord => MRI => millimeters  (NEW VERSION: 12-Jan-2016)
