function F = in_fread_eyelink(sFile, iEpoch, SamplesBounds, iChannels)
% IN_FREAD_EYELINK:  Read a block of recordings from a EyeLink eye tracker file (.edf).
%
% USAGE:  F = in_fread_eyelink(sFile, iEpoch=1, SamplesBounds=[all], iChannels=[all])
...
for i = 1:length(iChannels)
    ...
    F(i,:) = double(Trials(iEpoch).Samples.(chname)(iSamples));
end
