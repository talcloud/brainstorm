...
MAX_AVERAGE_BINS = 8;
SensorTypes ={...
      'Ref magnetometer',... % 0  Reference magnetometer channel          (Set to 5002 to use the definition in coil_def.dat)
      'Ref gradiometer' ,... % 1  Reference 1st-order gradiometer channel (Set to 5003 or 5004 to use the definition in coil_def.dat)
...
% === CTF SYSTEM ===
    ...
    % => so, they're not contiguous to the true reference channel list
    diffref = diff(irefsens);
...
for chan = 1:nchan
    ...
    if ismember(chan, [imegsens, irefsens])
        ...
        if isempty(Channel(chan).Loc)
            ...
            % Consider that the center of the head should be around (0, 0, -0.25)
            headCenter = [0,0,-0.25];
        else
            % The head center is at (0,0,0) because we are already in head coordinates
            headCenter = [0,0,0];
        end
        % Reorient along the outward pointing normal
        loc =  Channel(chan).Loc';
        orient = Channel(chan).Orient';
        for iCoil = 1:nbCoil
            ps = sum((loc(iCoil,:) - headCenter) .* orient(iCoil,:), 2);
            Channel(chan).Orient(:,iCoil) = (sign(ps) * ones(1,3) .* orient(iCoil,:))';
        end
    end
