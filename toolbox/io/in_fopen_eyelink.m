...
%% ===== CREATE CHANNEL FILE =====
% List of channels
hdr.chnames = {'flags','px','py','hx','hy','pa','gx','gy','rx','ry','input','buttons','htype','hdata','errors', ...
               'gxvel','gyvel','hxvel','hyvel','rxvel','ryvel','fgxvel','fgyvel','fhxvel','fhyvel','frxvel','fryvel'};
hdr.chtypes = {'STATUS','EYE','EYE','EYE','EYE','EYE','EYE','EYE','EYE','EYE','STATUS','STATUS','STATUS','STATUS','STATUS', ...
...
for i = 1:nEpochs
    sFile.epochs(i).times   = double([hdr.Headers(i).starttime, hdr.Headers(i).endtime]) ./ 1000; % 1 timestamp = 1 millisecond
    sFile.epochs(i).samples = round(sFile.epochs(i).times .* double(hdr.Headers(i).rec.sample_rate));
...
sFile.prop.sfreq   = double(hdr.Headers(1).rec.sample_rate);
...
for i = 1:nEpochs
    ...
    if ~isempty(Trials(i).Microsaccades)
    ...
    if ~isempty(Trials(i).Events.message)
...
for i = 1:length(events)
    if ~isempty(events(i).times)
        events(i).samples = round(events(i).times .* sFile.prop.sfreq);
        events(i).times = events(i).samples ./ sFile.prop.sfreq;
    end
end
% Import this list
