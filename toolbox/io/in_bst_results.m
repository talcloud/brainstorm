...
elseif LoadFull && ismember('Time', FieldsToRead) && isempty(Results.Time) && ~isempty(DataFileFull) && file_exist(DataFileFull)
    ...
    Results.Time = DataMat.Time;
% Get the number of averages for dSPM
elseif isfield(Results, 'Function') && strcmpi(Results.Function, 'dspm') && ~isempty(DataFileFull) && file_exist(DataFileFull)
...
if isfield(Results, 'Function') && strcmpi(Results.Function, 'dspm') && isfield(Results, 'ImageGridAmp') && ~isempty(DataMat)
    if (DataMat.nAvg ~= Results.nAvg)
        disp(sprintf('BST> Loading dSPM maps: scaling the values to match the number of trials averaged (%d => %d)', Results.nAvg, DataMat.nAvg));
