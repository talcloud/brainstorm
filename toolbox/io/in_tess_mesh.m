...
for ii = 1:mesh_time    
    ...
    vertex{ii} = reshape(vertex{ii},3,nb_vertex{ii})' /1000; % in mm
    % Normals
...
%% ===== CONVERT IN BRAINSTORM STRUCTURE =====
for iTess = 1:mesh_time
    TessMat(iTess).Vertices = double(vertex{iTess});  % FT 11-Jan-10: Remove "single"
    TessMat(iTess).Faces = double(faces{iTess} + 1);  % FT 11-Jan-10: Remove "single"
end
