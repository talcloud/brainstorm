...
list_methods = {'bst-fft', 'bst-fft-fir', 'bst-filtfilt-fir', 'bst-filtfilt-iir', 'bst-sos', 'fieldtrip_butter'};
% Intialize arrays
...
for i = 1:length(list_methods);
    try
        ...
        % Resample
        tic;
        a{i} = process_bandpass('Compute', a_init, sfreq, HighPass, LowPass, method); 
        el{i} = toc;
        % Plot initial signal
        if ~isFigCreated
            isFigCreated = 1;
