...
%         |- Brainstorm:  pmask1 = bst_stat_thresh(pmap, StatThreshOptions);
%                         StatThreshOptions.pThreshold = 0.05;
%                         StatThreshOptions.Correction = 'fdr';
%                         StatThreshOptions.Control    = [1 2 3];
%         |- Matlab:      pmask2 = mafdr(pmap,'BHFDR',.05) < .05;
%         |- FieldTrip:   pmask3 = fdr(pmap, .05) > .05;           % This produces different results than the other methods
...
switch (StatThreshOptions.Correction)
    ...
    case 'fdr'
        ...
        % Produces more significant values than the FieldTrip equivalent:  pmask = fdr(pmap,pThresh) < pThresh;
