...
for iFile = 1:nBlocks
    ...
    fileCov2 = DataMat.nAvg .* (DataMat.F(iGoodChan,iTimeBaseline).^2 * DataMat.F(iGoodChan,iTimeBaseline)'.^2);
...
switch lower(Options.NoiseCovMethod)
    ...
    case 'diag'
        ...
        FourthMoment = diag(diag(FourthMoment));
    otherwise
...
NoiseCovMat = bst_history('add', NoiseCovMat, 'compute', sprintf('Computed based on %d files (%d samples): Baseline [%1.2f, %1.2f]ms, %s, %s', ...
                          nBlocks, nSamplesTotal, Options.Baseline * 1000, Options.RemoveDcOffset, Options.NoiseCovMethod));
% Save in database
...
%% ===== READ RECORDINGS BLOCK =====
    function [DataMat, iTimeBaseline] = ReadRecordings()
        ...
        if ~isRaw
            ...
            % Read raw block
            [DataMat.F, DataMat.Time] = panel_record('ReadRawBlock', sFile, ChannelMat, DataMats(iFile).iEpoch, DataMats(iFile).Time([1,end]), 0, 1, 'no');
