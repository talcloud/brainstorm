...
iEegNoLoc = [];
iEegLoc = [];
iCheck = channel_find(ChannelMat.Channel, 'EEG, Fiducial');
    ...
    switch(chName)
        ...
        otherwise
            ...
            elseif ~isempty(strfind(chName, 'sfh')) || ~isempty(strfind(chName, 'sfl')) || ~isempty(strfind(chName, 'SL_')) || ~isempty(strfind(chName, 'SP_')) 
                iDelChan = [iDelChan, iChan];
                ChannelMat.HeadPoints.Loc(:,end+1) = ChannelMat.Channel(iChan).Loc;
                ChannelMat.HeadPoints.Label{end+1} = ChannelMat.Channel(iChan).Name;
