...
function ViewTopography(hFig)
    ...
    switch(FigureType)
        ...
        case {'3DViz', 'DataTimeSeries', 'ResultsTimeSeries'}
            ...
            if ~ismember(Modality, {'MEG','MEG GRAD','MEG MAG','EEG','ECOG','SEEG','NIRS'}) && ~isempty(DataFile)
    ...
    if ~isempty(DataFile) && ~isempty(Modality)
        if ismember(Modality, {'ECOG', 'SEEG'})
            % 3D figure: plot topography in the same figure
            if isequal(FigureType, '3DViz')
                view_topography(DataFile, Modality, '3DElectrodes', [], [], hFig);
            % Other types of figures: Create new figure
            else
                view_topography(DataFile, Modality, '3DElectrodes');
            end
        else
            view_topography(DataFile, Modality, '2DSensorCap');
        end
    end
end
