        ...
        switch (GlobalData.DataSet(iDS).Results(iResult).HeadModelType)
            ...
            case 'mixed'
                ...
                for i = 1:length(sScouts)
                    if (sScouts(i).Region(2) == 'V') || (sScouts(i).Region(2) == 'S')
                        GridLoc = cat(1, GridLoc, GlobalData.DataSet(iDS).Results(iResult).GridLoc(sScouts(i).GridRows,:));
                    end
                end
            otherwise
...
function LoadChannelFile(iDS, ChannelFile)
    ...
    if ~isempty(ChannelFile)
        ...
        if any(ismember({'ECOG', 'SEEG'}, {ChannelMat.Channel.Type}))
            panel_montage('AddAutoMontages', iDS, ChannelMat);
        end
...
function F = LoadRecordingsRaw(iDS)
    ...
    % Read data block
    F = panel_record('ReadRawBlock', GlobalData.DataSet(iDS).Measures.sFile, ChannelMat, iEpoch, TimeRange, 1, RawViewerOptions.UseCtfComp, RawViewerOptions.RemoveBaseline);
end
...
function F = FilterLoadedData(F, sfreq)
    ...
    if (isHighPass || isLowPass) && (nTime > 2)
        ...
        % Filter data
        F = process_bandpass('Compute', F, sfreq, HighPass, LowPass, [], isMirror);
    end
    ...
    if isSinRemoval && ~isempty(GlobalData.VisualizationFilters.SinRemovalValue) && (nTime > 2)
        ...
        %F = process_sin_remove('Compute', F, sfreq, GlobalData.VisualizationFilters.SinRemovalValue);
    end
end
...
function [iDS, iTimefreq, iResults] = LoadTimefreqFile(TimefreqFile, isTimeCheck, isLoadResults, isForceReload, PacOption)
    ...
    if ~isStat
        % Load .Mat
        TimefreqMat = in_bst_timefreq(TimefreqFile, 0, 'TF', 'Time', 'Freqs', 'DataFile', 'DataType', 'Comment', 'TimeBands', 'RowNames', 'RefRowNames', 'Measure', 'Method', 'Options', 'ColormapType', 'DisplayUnits', 'Atlas', 'SurfaceFile', 'sPAC', 'GridLoc', 'GridAtlas');
...
%         end
        ...
        % Load stat matrix
        TimefreqMat = in_bst_timefreq(TimefreqFile, 0, 'pmap', 'tmap', 'df', 'Time', 'Freqs', 'DataFile', 'DataType', 'Comment', 'TF', 'TimeBands', 'RowNames', 'RefRowNames', 'Measure', 'Method', 'Options', 'ColormapType', 'DisplayUnits', 'Atlas', 'SurfaceFile', 'sPAC', 'GridLoc', 'GridAtlas', 'Correction', 'StatClusters');
