...
%               bst_colormaps('CreateColormapMenu',   jMenu,    dataType)
...
%               bst_colormaps('SetMaxMode',           ColormapType, maxmode)
%               bst_colormaps('SetMaxCustom',         ColormapType, newmin, newmax)
%               bst_colormaps('SetMaxCustom',         ColormapType)
...
% USAGE: SetMaxCustom(ColormapType, newmin, newmax)
%        SetMaxCustom(ColormapType)
function SetMaxCustom(ColormapType, newmin, newmax)
    global GlobalData;
    % Get target colormap
...
%  ============================================================================
%% ====== CREATE COLORMAP MENU ======
% USAGE: CreateColormapMenu(jMenu, ColormapType)
function CreateColormapMenu(jMenu, ColormapType)
    import javax.swing.*;
    import java.awt.*;
    ...
    if ~strcmpi(ColormapType, 'Anatomy') && ~strcmpi(ColormapType, 'Time') && ~strcmpi(ColormapType, 'Overlay')
        ...
        CreateSeparator(jMenu, isPermanent);
        % Options : Maximum
        jRadioGlobal = gui_component('RadioMenuItem', jMenu, [], 'Maximum: Global',    [], [], @(h,ev)SetMaxMode(ColormapType, 'global'), []);
        jRadioLocal  = gui_component('RadioMenuItem', jMenu, [], 'Maximum: Local',     [], [], @(h,ev)SetMaxMode(ColormapType, 'local'), []);
        jRadioCustom = gui_component('RadioMenuItem', jMenu, [], 'Maximum: Custom...', [], [], @(h,ev)SetMaxMode(ColormapType, 'custom'), []);
        switch lower(sColormap.MaxMode)
...
function CreateAllMenus(jMenu, hFig, isDynamic) %#ok<DEFNU>
    ...
    if isempty(hFig) || ismember('anatomy', AllTypes)
        ...
        if isDynamic
            java_setcb(jMenuColormap, 'MenuSelectedCallback', @(h,ev)CreateColormapMenu(ev.getSource(), 'anatomy'));
        else
            CreateColormapMenu(jMenuColormap, 'anatomy');
        end
    end
    if isempty(hFig) || ismember('eeg', AllTypes)
        ...
        if isDynamic
            java_setcb(jMenuColormap, 'MenuSelectedCallback', @(h,ev)CreateColormapMenu(ev.getSource(), 'eeg'));
        else
            CreateColormapMenu(jMenuColormap, 'eeg');
        end
    end
    if isempty(hFig) || ismember('meg', AllTypes)
        ...
        if isDynamic
            java_setcb(jMenuColormap, 'MenuSelectedCallback', @(h,ev)CreateColormapMenu(ev.getSource(), 'meg'));
        else
            CreateColormapMenu(jMenuColormap, 'meg');
        end
    end
    if isempty(hFig) || ismember('source', AllTypes)
        ...
        if isDynamic
            java_setcb(jMenuColormap, 'MenuSelectedCallback', @(h,ev)CreateColormapMenu(ev.getSource(), 'source'));
        else
            CreateColormapMenu(jMenuColormap, 'source');
        end
    end
    if isempty(hFig) || ismember('stat1', AllTypes)
        jMenuColormap = gui_component('Menu', jMenu, [], [spre 'Stat 1'], IconLoader.ICON_COLORMAP_STAT, '', [], []);
        if isDynamic
            java_setcb(jMenuColormap, 'MenuSelectedCallback', @(h,ev)CreateColormapMenu(ev.getSource(), 'stat1'));
        else
            CreateColormapMenu(jMenuColormap, 'stat1');
        end
    end
    if isempty(hFig) || ismember('stat2', AllTypes)
        jMenuColormap = gui_component('Menu', jMenu, [], [spre 'Stat 2'], IconLoader.ICON_COLORMAP_STAT, '', [], []);
        if isDynamic
            java_setcb(jMenuColormap, 'MenuSelectedCallback', @(h,ev)CreateColormapMenu(ev.getSource(), 'stat2'));
        else
            CreateColormapMenu(jMenuColormap, 'stat2');
        end
    end
    if isempty(hFig) || ismember('time', AllTypes)
        jMenuColormap = gui_component('Menu', jMenu, [], [spre 'Time'], IconLoader.ICON_COLORMAP_TIME, '', [], []);
        if isDynamic
            java_setcb(jMenuColormap, 'MenuSelectedCallback', @(h,ev)CreateColormapMenu(ev.getSource(), 'time'));
        else
            CreateColormapMenu(jMenuColormap, 'time');
        end
    end
    if isempty(hFig) || ismember('timefreq', AllTypes)
        ...
        if isDynamic
            java_setcb(jMenuColormap, 'MenuSelectedCallback', @(h,ev)CreateColormapMenu(ev.getSource(), 'timefreq'));
        else
            CreateColormapMenu(jMenuColormap, 'timefreq');
        end
    end
    if isempty(hFig) || ismember('connect1', AllTypes)
        ...
        if isDynamic
            java_setcb(jMenuColormap, 'MenuSelectedCallback', @(h,ev)CreateColormapMenu(ev.getSource(), 'connect1'));
        else
            CreateColormapMenu(jMenuColormap, 'connect1');
        end
    end
    if isempty(hFig) || ismember('connectn', AllTypes)
        ...
        if isDynamic
            java_setcb(jMenuColormap, 'MenuSelectedCallback', @(h,ev)CreateColormapMenu(ev.getSource(), 'connectn'));
        else
            CreateColormapMenu(jMenuColormap, 'connectn');
        end
    end
    if isempty(hFig) || ismember('pac', AllTypes)
        jMenuColormap = gui_component('Menu', jMenu, [], [spre 'PAC'], IconLoader.ICON_COLORMAP_PAC, '', [], []);
        if isDynamic
            java_setcb(jMenuColormap, 'MenuSelectedCallback', @(h,ev)CreateColormapMenu(ev.getSource(), 'pac'));
        else
            CreateColormapMenu(jMenuColormap, 'pac');
        end
    end
    if isempty(hFig) || ismember('image', AllTypes)
        ...
        if isDynamic
            java_setcb(jMenuColormap, 'MenuSelectedCallback', @(h,ev)CreateColormapMenu(ev.getSource(), 'image'));
        else
            CreateColormapMenu(jMenuColormap, 'image');
        end
    end
end
...
function SetMaxMode(ColormapType, maxmode)
    % Check values
