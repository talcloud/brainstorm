            ...
            case {'custom', 'custom2'}
                ...
                try
                    if strcmpi(sProcesses(iProc).Category, 'custom')
                        ...
                        [OutputFiles, OutputFiles2] = sProcesses(iProc).Function('NoCatch', 'Run', sProcesses(iProc), sInputs, sInputs2);
                    end
                catch
                    strError = bst_error();
                    bst_report('Error', sProcesses(iProc), sInputAll, strError);
...
function OutputFile = ProcessFilter(sProcess, sInput)
    ...
    if isRaw
        ...
        if 0 % ProcessOptions.UseOrigFolder
%             % Use original file format
%             if ProcessOptions.UseOrigFormat
%                 switch (sFileIn.format)
%                     case 'FIF'
%                         RawFileOut = bst_fullfile(rawPathIn, [rawBaseIn, fileTag, '.fif']);
%                     case {'CTF', 'CTF-CONTINUOUS'}
%                         RawFileOut = bst_fullfile(rawPathIn, [rawBaseIn, fileTag, '.ds']);
%                     otherwise
%                         bst_report('Error', sProcess, sInput, ['Only FIF and CTF files can be saved in original format.' 10 'Go to the Brainstorm preferences and uncheck the option "Save in original file format".']);
%                         return;
%                 end
%                 RawFileFormat = sFileIn.format;
%             % Use original folder, but Brainstorm-binary format
%             else
%                 RawFileOut = bst_fullfile(rawPathIn, [rawBaseIn, fileTag, '.bst']);
%                 RawFileFormat = 'BST-BIN';
%             end
%             % Loop to find a file that matches both the unicity in the database and the raw folder
%             isLoop = 1;
%             while isLoop
%                 % Make output filename unique
%                 RawFileOut = file_unique(RawFileOut);
%                 % Create condition based on the output filename
%                 [rawPathOut, rawBaseOut, rawExtOut] = bst_fileparts(RawFileOut);
%                 newStudyPath = bst_fullfile(ProtocolInfo.STUDIES, sInput.SubjectName, ['@raw', rawBaseOut]);
%                 % If the condition already exists, iterate
%                 if file_exist(newStudyPath)
%                     newStudyPath = file_unique(newStudyPath);
%                     [condPath, condBase, condExt] = bst_fileparts(newStudyPath);
%                     newBaseOut = strrep([condBase, condExt], '@raw', '');
%                     RawFileOut = strrep(RawFileOut, rawBaseOut, newBaseOut);
%                 else
%                     isLoop = 0;
%                 end
%             end
    ...
    for iBlockRow = 1:nBlockRow
        ...
        for iBlockCol = 1:nBlockCol
            ...
            if isFirstLoop
                ...
                if isRaw
                    % Create new file in original format
                    if 0 % ProcessOptions.UseOrigFormat
                       %  [sFileOut, errMsg] = CreateRawOut(sFileIn, RawFileOut, ImportOptions);
...
function OutputFile = ProcessFilter2(sProcess, sInputA, sInputB)
    ...
    if isAbsolute
        ...
        % Add tags
        fileTag = [fileTag, '_', strTag];
        sInputA.Comment = [sInputA.Comment, ' | ', strTag];
        sInputB.Comment = [sInputB.Comment, ' | ', strTag];
    end
    ...
    if isfield(sMatOut, 'DataFile') && (length(uniqueDataFile) > 1) && ~ismember(func2str(sProcess.Function), {'process_zscore_ab', 'process_zscore_dynamic_ab'})
        sMatOut.DataFile = [];
    end
    % If data + changed data type
...
function OutputFiles = ProcessStat(sProcess, sInputA, sInputB)
    ...
    iType = find(strcmpi(sInputA(1).FileType, sProcess.InputTypes));
    ...
    % If not available: stat from the original file
        ...
        OutputFileType = ['p', sInputA(1).FileType];
    end
...
function [sStudy, iStudy, Comment, uniqueDataFile] = GetOutputStudy(sProcess, sInputs, intraCondName, isCreateCond)
    ...
    if isempty(Comment)
        ...
        elseif ismember(sInputs(1).FileType, {'timefreq','results'}) && ~isempty(sInputs(1).DataFile) && ismember(file_gettype(sInputs(1).DataFile), {'data','matrix'})
            ...
            for iFile = 1:length(sInputs)
                % Find parent file in the database
                [sStudyComment,tmp__,iData]  = bst_get('AnyFile', sInputs(1).DataFile);
