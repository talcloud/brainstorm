...
function sProcess = DefineStatOptions(sProcess)
    ...
    % === ABSOLUTE VALUE
    sProcess.options.abs.Comment    = 'Test absolute values';
    sProcess.options.abs.Type       = 'checkbox';
    sProcess.options.abs.Value      = 0;
    % === AVERAGE OVER TIME
...
function OPT = GetStatOptions(sProcess)
    ...
    OPT.isAbsolute  = sProcess.options.abs.Value;
    if OPT.isAbsolute
        strAbs = ' abs';
    else
        strAbs = '';
    ...
    switch (sProcess.options.statistictype.Value)
        case 1,  OPT.StatisticType  = 'indepsamplesT';
        case 2,  OPT.StatisticType  = 'depsamplesT';
    end
    ...
    switch (sProcess.options.correctiontype.Value{1})
        case 1,  OPT.Correction = 'no';          OPT.strComment = ['Perm t-test' strAbs ' [uncorrected]'];
        case 2,  OPT.Correction = 'cluster';     OPT.strComment = ['Perm t-test' strAbs ' [cluster]'];
        case 3,  OPT.Correction = 'bonferroni';  OPT.strComment = ['Perm t-test' strAbs ' [bonferroni]'];
        case 4,  OPT.Correction = 'fdr';         OPT.strComment = ['Perm t-test' strAbs ' [fdr]'];
        case 5,  OPT.Correction = 'max';         OPT.strComment = ['Perm t-test' strAbs ' [max]'];
        case 6,  OPT.Correction = 'holm';        OPT.strComment = ['Perm t-test' strAbs ' [holm]'];
        case 7,  OPT.Correction = 'hochberg';    OPT.strComment = ['Perm t-test' strAbs ' [hochberg]'];
    end
...
function sOutput = Run(sProcess, sInputsA, sInputsB) %#ok<DEFNU>
    ...
    % Output channel flag
    if ~isMatrix
        sOutput.ChannelFlag  = -1 * ones(1, nChannels);
