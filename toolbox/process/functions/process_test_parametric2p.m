...
% PROCESS_TEST_PARAMETRIC2P: Parametric dependent(=paired) two-sample tests.
...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    % === TEST: type
    sProcess.options.test_type.Comment = {['<B>Paired Student''s t-test</B> <BR>' ...
                                           't = mean(A-B) ./ std(A-B) .* sqrt(n) &nbsp;&nbsp;&nbsp;&nbsp; <FONT COLOR="#777777">df=n-1</FONT>']; ...
                                          'ttest_paired'};    
%     sProcess.options.test_type.Comment = {['<B>Paired Student''s t-test</B> <BR>' ...
%                                            't = mean(A-B) ./ std(A-B) .* sqrt(n)' &nbsp;&nbsp;&nbsp;&nbsp; <FONT COLOR="#777777">df=n-1</FONT>], ...
%                                           ['<B>Paired Student''s t-test (rectified)</B> <BR>' ...
%                                            't = mean(|A-B|) ./ std(|A-B|) .* sqrt(n)' &nbsp;&nbsp;&nbsp;&nbsp; <FONT COLOR="#777777">df=n-1</FONT>]; ...
%                                           'ttest_paired', 'ttest_abspaired'};
