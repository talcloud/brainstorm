...
function OutputFiles = Run(sProcess, sInputsA, sInputsB)
    ...
    if isfield(sProcess.options, 'eventtime') && isfield(sProcess.options.eventtime, 'Value') && ~isempty(sProcess.options.eventtime.Value)
        ...
        if isRawA && ~isempty(evtName)
            ...
            for iOcc = 1:nOcc
                ...
                if isIgnoreBad && ~isempty(badSeg) && (~all((SamplesBounds(2) < badSeg(1,:)) | (SamplesBounds(1) > badSeg(2,:))))
                    bst_report('Info', sProcess, sInputsA(iFile), sprintf('Event %s #%d is in a bad segment: ignored...', evtName, iOcc));
                    continue;
    ...
    for iBlock = 1:length(F)
        ...
        % Filter recordings
        if ~isempty(BandPass) && ~all(BandPass == 0)
            F{iBlock}(iChannels,:) = process_bandpass('Compute', F{iBlock}(iChannels,:), sfreq, BandPass(1), BandPass(2));
        end
    ...
    nTime = size(F,2);
    nChan = size(F,1);
    if ~isempty(strfind(Method, 'ICA')) && (nTime / nChan^2 < 10)
        bst_report('Warning', sProcess, [], ['There is probably not enough data for a correct ICA decomposition.' 10 ...
...
%         % === INFOMAX_TEST ===
%         case 'ICA_infomax_Test'
%             % Concatenate all the loaded data
%             F = cat(3, F{:});
%             % Copy variable to base workspace
%             assignin('base', 'AllMat', F);
%         %     addpath('C:\Work\Dev\Divers\eeglab13_4_4b');
%         %     eeglab; 
%             % Import in EEGLAB
