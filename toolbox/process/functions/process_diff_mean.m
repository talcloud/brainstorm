...
function OutputFiles = Run(sProcess, sInputsA, sInputsB) %#ok<DEFNU>
    ...
    isVariance = 0;
    [StatA, MessagesA, iAvgFileA] = bst_avg_files({sInputsA.FileName}, [], Function, isVariance, isWeighted, isMatchRows);
    [StatB, MessagesB, iAvgFileB] = bst_avg_files({sInputsB.FileName}, [], Function, isVariance, isWeighted, isMatchRows);
    ...
    elseif isResults && strcmpi(Function, 'norm') && (sMat.nComponents ~= 1)
        sMat = process_source_flat('Compute', sMat, 'rms');
    end
    ...
    if any(StatA.nGoodSamples(1) ~= StatA.nGoodSamples) && isfield(StatA, 'RowNames') && ~isempty(StatA.RowNames)
