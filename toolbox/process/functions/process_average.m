...
%                            OutputFile = process_average('AverageFiles', sProcess, sInputs, KeepEvents, isScaleDspm, isWeighted, isMatchRows)
...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.options.avg_func.Comment = {'Arithmetic average:  <FONT color="#777777">mean(x)</FONT>', ...
                                         ...
                                         'Standard error:  <FONT color="#777777">sqrt(var(x))/sqrt(N)</FONT>', ...
...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    for i = 1:length(iGroups)
        ...
        OutputFiles{end+1} = AverageFiles(sProcess, sInputs(iGroups{i}), KeepEvents, isScaleDspm, isWeighted, isMatchRows);
    end
end
...
%% ===== AVERAGE FILES =====
function OutputFile = AverageFiles(sProcess, sInputs, KeepEvents, isScaleDspm, isWeighted, isMatchRows)
    % Parse inputs
    ...
    % Compute average
    [Stat, Messages, iAvgFile, Events] = bst_avg_files({sInputs.FileName}, [], Function, isVariance, isWeighted, isMatchRows, 1);
    ...
    if any(Stat.nGoodSamples(1) ~= Stat.nGoodSamples) && isfield(Stat, 'RowNames') && ~isempty(Stat.RowNames)
