...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.Index       = 42;
    sProcess.Description = 'http://neuroimage.usc.edu/brainstorm/Tutorials/StimDelays#Detection_of_the_analog_triggers';
...
function evt = Compute(F, TimeVector, EventSamps, OPTIONS, Fmask)
    ...
    if ~isempty(OPTIONS.highpass) || ~isempty(OPTIONS.lowpass)
        F = process_bandpass('Compute', F, sFreq, OPTIONS.highpass, OPTIONS.lowpass);
    end
