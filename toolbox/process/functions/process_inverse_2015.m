function varargout = process_inverse_2015( varargin )
% PROCESS_INVERSE_2015: Compute an inverse model.
...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.Comment     = 'Compute sources [2015]';
    sProcess.FileTag     = '';
    ...
    if isempty(ALLOW_2015) || ~ALLOW_2015
        sProcess.options.warning.Comment = '<B><FONT color="#CC0000">Warning: This interface is under development.<BR>Use at your own risk.</B></FONT><BR><BR>';
    ...
    % Options: MNE options
    sProcess.options.inverse.Comment = {'panel_inverse_2015', 'Source estimation options: '};
    sProcess.options.inverse.Type    = 'editpref';
    sProcess.options.inverse.Value   = bst_inverse_linear_2015();
end
...
%          Francois Tadel, John Mosher, 2009-2015
%
function [OutputFiles, errMessage] = Compute(iStudies, iDatas, OPTIONS)
    ...
    if OPTIONS.DisplayMessages
        ...
        sMethod = gui_show_dialog('Compute sources', @panel_inverse_2015, 1, [], AllMod, isShared, HeadModelType);
        if isempty(sMethod)
            return;
        end
    ...
    methodTag = panel_inverse_2015('GetMethodComment', OPTIONS.InverseMethod, OPTIONS.InverseMeasure);
    ...
    elseif ~strcmpi(OPTIONS.InverseMethod, 'mem')
        switch (OPTIONS.SourceOrient{1})
            ...
            case 'optimal',    strOptions = 'Optimal';
    ...
    for iEntry = 1:length(iStudies)
        ...
        if strcmpi(OPTIONS.InverseMethod, 'lcmv') && isempty(DataCovMat)
            ...
            break;
        % Estimated SNR: Require a data covariance matrix
        elseif strcmpi(OPTIONS.SnrMethod, 'estimate') && isempty(DataCovMat)
            errMessage = [errMessage 'You need to calculate a data covariance before using the option "Estimate SNR from data".' 10];
            break;
        end
        ...
        % Load head model
        HeadModel = in_headmodel_bst(HeadModelFile, 0, 'Gain', 'GridLoc', 'GridOrient', 'GridAtlas', 'SurfaceFile', 'MEGMethod', 'EEGMethod', 'ECOGMethod', 'SEEGMethod', 'HeadModelType');
        ...
        switch( OPTIONS.InverseMethod )       
            case {'minnorm', 'gls', 'lcmv'}
                ...
                % NOTE: The output HeadModel param is used here in return to save LOTS of memory in the bst_inverse_linear_2015 function,
                ...
                [Results, OPTIONS] = bst_inverse_linear_2015(HeadModel, OPTIONS);
            case 'mem'
        ...
        ResultsMat.Comment       = [OPTIONS.Comment ' 2015'];
