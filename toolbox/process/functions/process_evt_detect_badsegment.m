...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.Index       = 46;
    sProcess.Description = 'http://neuroimage.usc.edu/brainstorm/Tutorials/BadSegments#Automatic_detection';
...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    for iFile = 1:length(sInputs)
        ...
        for ff = 1:size(freqs,1)
            ...
            % Filter file
            Fbp = process_bandpass('Compute', Fraw, sfreq, lowCut, highCut,[],1);
            if isempty(Fbp)
                return;
            end
            ...
            for ii=1:length(winSt)-1
                ...
                if isempty(F)
                 ...
                 % Apply filtering if requested, otherwise remove the DC
                F = process_bandpass('Compute', F, sfreq, lowCut, highCut,[],1);
                if isempty(F)
                    return;
                end
