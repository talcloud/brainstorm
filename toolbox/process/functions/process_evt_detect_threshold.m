...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    sProcess.options.units.Comment = {'None (10^0)', 'mV (10^-3)', 'uV (10^-6)', 'fT (10^-15)'};
    sProcess.options.units.Type    = 'radio';
    sProcess.options.units.Value   = 1;
    % Separator
    sProcess.options.separator2.Type = 'separator';
    sProcess.options.separator2.Comment = ' ';
    % Filter
    sProcess.options.label2.Comment = '<U><B>Filter signal before detection:</B></U>';
...
function evt = Compute(F, TimeVector, OPTIONS, Fmask)
    ...
    if ~isempty(OPTIONS.bandpass)
        F = process_bandpass('Compute', F, sFreq, OPTIONS.bandpass(1), OPTIONS.bandpass(2));
    end
