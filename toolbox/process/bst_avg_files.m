function [Stat, Messages, iOutFiles, AllEvents] = bst_avg_files(FilesListA, FilesListB, Function, isVariance, isWeighted, isMatchRows, isPercent)
% BST_AVG_FILES: Compute statistics on one or two files sets.
%
% USAGE:  [Stat, Messages, iOutFiles, AllEvents] = bst_avg_files(FilesListA, FilesListB, Function, isVariance, isWeighted, isMatchRows, isPercent)
%         [Stat, Messages, iOutFiles, AllEvents] = bst_avg_files(FilesListA, FilesListB, Function, isVariance, isWeighted, isMatchRows, isPercent)
%
% INPUT:
...
if isMatchRows && ismember(file_gettype(FilesListA{1}), {'matrix','timefreq','pmatrix','ptimefreq'})
    ...
    if isempty(DestRowNames)
        Messages = [Messages, 'Could not find a common list of rows: Trying to average directly the matrices...' 10];
    end
else
    DestRowNames = [];
end
...
for iFile = 1:nFiles
    ...
    elseif (strcmpi(matName, 'F') || strcmpi(matName, 'tmap')) && isfield(sMat, 'ChannelFlag') && ~isempty(sMat.ChannelFlag)
    ...
    if isDiff
        ...
        switch (Function)
            case {'mean', 'meandiffnorm'},                 matValues = matValues - double(sMat2.(matName));
            case 'rms',                                    matValues = matValues.^2 - double(sMat2.(matName)).^2;
            case {'abs','norm','normdiff','normdiffnorm'}, matValues = abs(matValues) - abs(double(sMat2.(matName)));
        end
