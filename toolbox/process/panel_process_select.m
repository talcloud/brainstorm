...
%  =========================================================================
    ...
    function [sProcesses, iSelProc] = GetAvailableProcesses(DataType, procFiles, nCurrentProc, nInputsCur)
...
%                 sProcessesAll(iProc).options.source_warning.Group   = 'input';
            ...
            if (procFiles(1) < sProcessesAll(iProc).nMinFiles) && ~((sProcessesAll(iProc).nMinFiles == 2) && (nCurrentProc >= 1))
                continue;
            end
    ...
    function ShowProcessMenu(AddMode)
        ...
        nCurrentProc = length(GlobalData.Processes.Current);
        % Get processes for this specific case
        [sProcesses, iSelProc] = GetAvailableProcesses(procDataType, procFiles, nCurrentProc, nInputsProc);
            ...
            switch (option.Type)
                ...
                case 'freqrange'
                    ...
                    if isempty(FreqList)
                        ...
                        gui_validate_text(jTextMin, [], jTextMax, bounds, 'Hz', precision, initStart, @(h,ev)OptionRange_Callback(iProcess, optNames{iOpt}, jCheck, jTextMin, jTextMax));
                        gui_validate_text(jTextMax, jTextMin, [], bounds, 'Hz', precision, initStop,  @(h,ev)OptionRange_Callback(iProcess, optNames{iOpt}, jCheck, jTextMin, jTextMax));
                    else
                        gui_validate_text(jTextMin, [], jTextMax, FreqList, 'Hz', precision, FreqList(1),   @(h,ev)OptionRange_Callback(iProcess, optNames{iOpt}, jCheck, jTextMin, jTextMax));
                        gui_validate_text(jTextMax, jTextMin, [], FreqList, 'Hz', precision, FreqList(end), @(h,ev)OptionRange_Callback(iProcess, optNames{iOpt}, jCheck, jTextMin, jTextMax));
