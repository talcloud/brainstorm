...
%         bst_report('Snapshot', hFig,           AnyFile,      Comment)
    ...
    try
        switch (SnapType)
            ...
            case 'sources'
                ...
                [sStudy, iStudy] = bst_get('ResultsFile', FileName);
                sSubject = bst_get('Subject', sStudy.BrainStormSubject);
                % Call surface viewer
                hFig = view_surface_data(sSubject.Surface(sSubject.iCortex).FileName, FileName, [], 'NewFigure');
            ...
            case 'ssp'
                ...
                for iProj = 1:length(ChannelMat.Projector)
                    proj = ChannelMat.Projector(iProj);
                    % Keep only the selected components
                    if ~isempty(proj.CompMask)
                        iSelComp = find(proj.CompMask);
                    else
                        iSelComp = 1;
                    end
                    % If projector not selected or no components: skip
                    if ~proj.Status || isempty(iSelComp)
                        continue;
                    end
                    % Loop on the components
                    for iComp = 1:length(iSelComp)
                        % Current component
                        comp = proj.Components(:,iComp);
                        ...
                        for iMod = 1:length(AllMod)
                            ...
                            iChan = good_channel(ChannelMat.Channel, DataMat.ChannelFlag, AllMod{iMod});
                            % Plot component topography for current modality
                            hFig(end+1) = view_topography(FileName, AllMod{iMod}, '2DSensorCap', comp(iChan), [], 'NewFigure');
                            imgLegend{end+1} = [AllMod{iMod} ' #' num2str(iSelComp(iComp)) ' ' proj.Comment];
                        end
                    end
                end
...
function jReport = Open(ReportFile, isFullReport)
    ...
    if ~isempty(iProcess)
        FilesInterm = {};
        for i = 1:length(iProcess)
            FilesInterm = GetFilesList(Reports{iProcess(i),3}, 1, 1);
        end
        FilesInterm = setdiff(FilesInterm, FilesInit);
        FilesInterm = setdiff(FilesInterm, FilesFinal);
    else
        FilesInterm = {};
    end
    ...
    if ~isempty(iImages)
        ...
        for i = 1:length(iImages)
            ...
            imgFile = bst_fullfile(tmpDir, sprintf('img%04d.jpg', i));
            imwrite(img, imgFile);
            % Display image
            html = [html, Comment, '  --  ' 10];
            html = [html, PrintFilesList(FileNames, ProtocolInfo, 1)];
            html = [html, '<IMG src=''file:///' strrep(imgFile,'\','/') ''' /><BR><BR>'];
        end
    end
...
%     % Intermediate files
%     if ~isempty(FilesInterm)
%         html = [html '<H2>Intermediate files</H2>' 10];
%         if ~isempty(FilesInterm) && ~isequal(FilesInterm, {[]})
%             html = [html PrintFilesList(FilesInterm, ProtocolInfo, 0)];
%         else
%             html = [html '<FONT color=#555555>[No intermediate files]</FONT><BR>'];
%         end
%     end
    % Final files
...
function LinkClickCallback(h, ev)
    ...
    if (ev.getEventType() == ev.getEventType().ACTIVATED)
        ...
        % Force the exploration mode of the database to be "functionial / subject"
...
function Recall(target)
        ...
        if (isempty(sCurFiles) && ~isempty(FileNames)) || (~isempty(sCurFiles) && isstruct(sCurFiles) && ~isequal({sCurFiles.FileName}, FileNames))
            ...
            % Check for existing files
                ...
                if isempty(FileNames)
                    error('The input files do not exist anymore.');
                end
    ...
    elseif (sProcesses(1).nInputs == 2)
        ...
        if ~iscell(FileNames) || (length(FileNames) ~= 2) || ~iscell(FileNames{1})
            error('This pipeline cannot be reloaded.');
        end
