...
for i=1:length(sensor_contents)
    if strcmp(sensor_contents{i}.type,'element')
        if strcmp(sensor_contents{i}.name,'coil')
            ...
            coil_contents=get(lenaTree,children(lenaTree,sensor_contents{i}.uid));
            if length(coil_contents)==1
                temp=coil_contents;
                clear coil_contents;
                coil_contents{1}=temp;
                clear temp;
            end
            for j=1:length(coil_contents)
                switch coil_contents{j}.name
                    case 'position'
