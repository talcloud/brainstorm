  ...
  if (isempty(trial_dim) & isempty(trial_level))
    first_pos_trial=0;
      ...
      if ~isempty(frequency_level) & ~isempty(sensor_level) & ~isempty(time_level)
                %first_pos_trial=0;%(frequency_level*sensor_level*time_level);
               [rank_sensor,  rank_frequency]=getRankSensorFrequency(dim_names);
               if rank_sensor > rank_frequency
                   %(frequency_dim(i_dim_trial)-1)*(frequency_level*sensor_level*time_level);
                   ...
                   echantillons= [echantillons echantillon];
          ...
          else if ~isempty(sensor_level) & ~isempty(frequency_level)
                       %  first_pos_trial=(sensor_level*time_level);
                         first_pos_time=0;%1;
                        [k, echantillon]= Process_Sensor_Frequency_NotTime(fid,data_size, data_type,first_pos_trial, first_pos_frequency,frequency_dim,sensor_dim, frequency_level,sensor_level,k); 
                        echantillons= [echantillons echantillon];
                       ...
                       %  first_pos_trial=(sensor_level*time_level);
                        ...
                        echantillons= [echantillons echantillon];
                         ...
                         %   first_pos_trial=(frequency_level*time_level);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [k, echantillons]= Process_Frequency_Sensor(fid,data_size, data_type,first_pos_trial, first_pos_frequency, frequency_dim,sensor_dim, time_dim, k,frequency_level, time_level, sensor_level); % frequence/temps
...
function [k, echantillons]= Process_Sensor_Frequency_NotTime(fid,data_size, data_type,first_pos_trial, first_pos_frequency,frequency_dim,sensor_dim,frequency_level,sensor_level,k)   
...
echantillons=[];
        ...
        if ~isempty(sensor_dim) 
              ...
              for i_dim_sensor=1:limit_sensor
                  ...
                  first_pos_sensor=(sensor_dim(i_dim_sensor)-1)*(frequency_level);
             [k, echantillon]=    Process_Frequency(fid,data_size, data_type,first_pos_trial,first_pos_sensor,frequency_dim,'', k,frequency_level, '');
                echantillons=[echantillons echantillon];
