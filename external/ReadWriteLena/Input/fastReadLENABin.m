function F = fastReadLENABin(lena_file,dim_to_read)
...
%  
% Usage :
%   F=fastReadLENABin('/path/to/file/test.lena');
%       will return a matrix containing all the data
%
%   F=fastReadLENABin('/path/to/file/test.lena',{ 2 , [ ] , [5:12] });
...
dim_to_read='';
else if length(dim_to_read) < length(dimensions_names)
...
for i = 1:length(dim_to_read)
    ...
    else if   exist('time_dim') & i==time_dim
