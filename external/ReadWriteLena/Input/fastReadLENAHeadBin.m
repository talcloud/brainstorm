function [Header, F] = fastReadLENAHeadBin(lena_file,dim_to_read)
...
%    - Header ( structure )  : a structure containing the header information that can be
...
% 
% % Usage :
% %   F=fastReadLENAHeadBin('/path/to/file/test.lena');
%       will return a matrix containing all the data
%
%   F=fastReadLENAHeadBin('/path/to/file/test.lena',{ 2 , [ ] , [5:12] });
...
% switch(data_type)Header.data_type='floating';
%     case 'unsigned fixed'
%         data_type='uint';
%     case 'fixed'
%         data_type='int';
%     case 'floating'
%         data_type='float32';  Header.data_type='floating';
%         
%     otherwise
...
for i = 1:length(dim_to_read)
    ...
    else if   exist('time_dim') & i==time_dim & isempty(dim_to_read{i})
            if level(i) > 1
            dim_to_read{i}=0:level(i)-1;
            else dim_to_read{i}=0;
            end
        else if exist('time_dim') & i==time_dim & ~isempty(dim_to_read{i})
                dim_to_read{i}=0:length(dim_to_read{i})-1;
            end
...
%function [dim_to_read,leveldim, sensorList, sensorSample]=ProcessSensorDim(lena_tree, sensor_dim,dimensions_names, dim_to_read)
