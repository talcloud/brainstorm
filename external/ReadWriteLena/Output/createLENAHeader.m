...
if ~isempty(header) 
   [data_format,data_type,data_size, datablocks,sensorList,  sensorSamples, frequenciesList, frequencySamples ,time_samples,sample_rate, pre_trigger, history]= CheckArgs(header);
...
tree=insertTimeRange(tree,num2str(time_samples),num2str (sample_rate),num2str(pre_trigger));
 ...
 [path,name,ext] = fileparts(xml_file);
    if isempty(ext)
      xml_file=  strcat(xml_file,'.lena');
    end
