function  lenaTree= setDate(lenaTree,date)
...
if isempty( dateId )
    [lenaTree, date] = add( lenaTree, root(lenaTree), 'element', 'date' );
    lenaTree = add(lenaTree,dateId,'chardata',date);

else warning( 'Can t find any date parameter in the file' )
     date = '';
  return
end
