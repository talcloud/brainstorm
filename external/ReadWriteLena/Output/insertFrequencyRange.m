function tree=insertFrequencyRange(tree,frequencyList,frequencySamples)
...
i_frequency=find(strcmp(des_names,'frequency_range'));

if isempty(i_frequency)
    [tree, frequency_uid] = add( tree, des_uid, 'element', 'frequency_range' );
    tree = attributes(tree,'add',frequency_uid,'name','frequency');
else
    error('Description already contains a frequency_range element, delete it before any new assignement');
    return;
end



tree = setFrequencyList(tree, frequencyList);
tree = setFrequencySamples(tree, frequencySamples);
