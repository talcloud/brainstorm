function  status = WrireBinaryFolderLENA(MatData, lena_file,dim_to_write)
...
if ~ isempty(header_file)
    ...
    lena_path=lena_file;
    lena_file=fullfile(lena_path,header_file.name);
...
data_filename = strcat(lena_path,'/',lena_name,'.data');
else data_filename = strcat(lena_name,'.data');
end
...
%command2= [command2 'fwrite(data_fid,  cell2mat(MatData (' matrice_index  ') ),data_type);'];
command2= [command2 'fwrite(data_fid,  cell2mat(MatData  ),Mat_data_type);'];

%else if length (dim_to_write)>0
 %       command2= [command2 'fwrite(data_fid,  MatData (' matrice_index  ') ,data_type);'];
    else
