function bst_deploy_java_2012b(IS_BIN)
% BST_DEPLOY_JAVA - Brainstorm deployment script.
%
% USAGE:  bst_deploy_java_2012b(IS_BIN=0)
%
% INPUTS:
...
if IS_BIN
    ...
    clc
    % JDK folder
    jdkDir = 'C:\Program Files\Java\jdk1.6.0_45';
    % Set JAVA_HOME environment variable
    setenv('JAVA_HOME', jdkDir);
    % Get Matlab version
    ReleaseName = bst_get('MatlabReleaseName');
    % Javabuilder output
    compilerFile = fullfile(bstDir, 'deploy', 'bst_javabuilder_2012b.prj');
...
if IS_BIN
    ...
    % Copy "brainstorm3\deploy\bst_javabuilder_2012b" to "brainstorm3_deploy/bst_javabuilder"
    movefile(fullfile(bstDir, 'deploy', 'bst_javabuilder_2012b'), compilerDir);
    
    % === PACKAGING ===
    % Compiled jar
    compiledJar = fullfile(compilerOutputDir, 'bst_javabuilder_2012b.jar');
    ...
    fwrite(fid, ['Manifest-Version: 1.0' 13 10 ...
                 'Main-Class: bst_javabuilder_2012b.Run' 13 10 ...
    ...
    classFile = fullfile('org', 'brainstorm', 'file', 'SelectMcr2012b.class');
    ...
    delete(bstJar);
    system(['cd "' jarDir '" & "' jdkDir '\bin\jar.exe" cmf manifest.txt "' bstJar '" bst_javabuilder_2012b org com']);
end
